...
test("renders baseline, impact, ceiling, and ceiling changes", () => {
  ...
  const comment = renderComment({
    ...
    currentRun: {
      ...
      conclusion: "success",
      url: "https://github.com/boroworx/iskra/actions/runs/2",
    },
    baselineRun: {
      sha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
      matchesBase: true,
      url: "https://github.com/boroworx/iskra/actions/runs/1",
    },
  });
  ...
  assert.match(comment, /<!-- iskra-thread-transfer-report -->/);
  assert.match(
    comment,
    /<!-- iskra-thread-transfer-result-sha:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb -->/,
  );
});
...
test("resolves a fallback PR with a redacted head repo and exact main baseline", async () => {
  ...
  await resolve({
    github,
    context: {
      repo: { owner: "pingdotgg", repo: "iskra" },
      payload: {
        workflow_run: {
          id: 2,
          event: "pull_request",
          ...
          head_repository: { full_name: "boroworx/iskra" },
          conclusion: "success",
          pull_requests: [],
        },
      },
      ...
      paginate: async () => [
        {
          ...
          body: `<!-- iskra-thread-transfer-report -->\n<!-- iskra-thread-transfer-result-sha:${sha} -->`,
        },
      ],
      rest: {
        issues: {
