...
pub fn destination(windows: Vec<Window>, pid: u32, title: &str) -> Result<Option<Window>> {
    ...
    if matches.next().is_some() {
        return Err("More than one Iskra window matches the capture destination.".into());
    }
    Ok(found)
}

pub fn activate(pid: u32, title: &str) -> Result<()> {
    // Subscribe before looking up a newly mapped Iskra window, so no map/title event is missed.
    ...
    loop {
        ...
        if let Some(window) = destination(windows()?, pid, title)? {
            ...
            {
                return Err("Hyprland could not focus Iskra.".into());
            }
            return Ok(());
        }
        events.get_ref().set_read_timeout(Some(
            deadline
                ...
                .ok_or("Iskra did not become visible.")?,
        ))?;
...
mod tests {
    ...
    fn destination_requires_unique_process_and_title() {
        assert!(
            destination(vec![window(2, "Iskra")], 1, "Iskra")
                .unwrap()
                .is_none()
        );
        assert!(
            destination(vec![window(1, "other")], 1, "Iskra")
                .unwrap()
                .is_none()
        );
        assert!(destination(vec![window(1, "Iskra"), window(1, "Iskra")], 1, "Iskra").is_err());
        assert!(
            destination(vec![window(2, "Iskra"), window(1, "Iskra")], 1, "Iskra")
                .unwrap()
                .is_some()
        );
    }
