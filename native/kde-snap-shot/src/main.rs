...
#[zbus::interface(name = "sh.iskra.KdeCapture")]
impl ScriptReply {
    fn reply(
        &self,
        value: &str,
...
fn script(connection: &Connection, directory: &Path, body: &str) -> Result<String> {
    ...
    let name = format!("iskra-capture-{}", std::process::id());
    ...
    let source = format!(
        "function reply(value) {{ callDBus({destination}, '/sh/iskra/KdeCapture', 'sh.iskra.KdeCapture', 'Reply', JSON.stringify(value)); }}\ntry {{ {body} }} catch (error) {{ reply({{error: String(error)}}); }}"
    );
    ...
    connection
        .object_server()
        .remove::<ScriptReply, _>("/sh/iskra/KdeCapture")?;
...
fn check(connection: &Connection) -> Result<()> {
    ...
    let reply: zbus::Result<HashMap<String, OwnedValue>> = proxy.call(
        "CaptureWindow",
        &(
            "iskra-permission-check-not-a-window",
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("activate") if args.len() == 5 => {
            ...
            if serde_json::from_str::<serde_json::Value>(&value)?["activated"] != true {
                return Err("KDE could not identify the Iskra window to activate".into());
            }
