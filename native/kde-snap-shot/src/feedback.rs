...
const OBJECT: &str = "/sh/iskra/KdeCapture/Feedback";
...
#[zbus::interface(name = "sh.iskra.KdeCapture.Feedback")]
impl Bridge {
...
pub(super) fn run(connection: &Connection, directory: &Path, options: &str) -> Result<()> {
    ...
    let name = format!("iskra-capture-feedback-{}", std::process::id());
