...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(IskraProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "iskra.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("IskraProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid iskra.json", () =>
      Effect.gen(function* () {
        const loader = yield* IskraProjectFileLoader.IskraProjectFileLoader;
    ...
    it.effect("returns none when iskra.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* IskraProjectFileLoader.IskraProjectFileLoader;
        const cwd = yield* makeTempDir;
