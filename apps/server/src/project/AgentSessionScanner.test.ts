...
import { symlinksSupported } from "@iskra/shared/testing/symlinks";
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        ServerConfig.layerTest(
          ...
          input.configBaseDir ?? { prefix: "iskra-scanner-config-" },
        ),
...
it.layer(NodeServices.layer)("AgentSessionScanner", (it) => {
  describe("scan", () => {
    it.effect("reads Claude project cwds from transcripts, newest first", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("iskra-claude-home-");
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const olderWorkspace = yield* makeTempDir("iskra-workspace-older-");
        const newerWorkspace = yield* makeTempDir("iskra-workspace-newer-");
    ...
    it.effect("groups Codex rollouts by cwd across date directories", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("iskra-claude-home-");
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const workspace = yield* makeTempDir("iskra-workspace-");
        const otherWorkspace = yield* makeTempDir("iskra-workspace-other-");
    ...
    it.effect("returns the imported project ID through a realpath alias", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const workspace = yield* makeTempDir("iskra-workspace-");
        const linkParent = yield* makeTempDir("iskra-scanner-links-");
    ...
    it.effect("keeps case variants distinct when the filesystem identities differ", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("iskra-claude-home-");
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const backingUpper = yield* makeTempDir("iskra-backing-upper-");
        const backingLower = yield* makeTempDir("iskra-backing-lower-");
        const aliasParent = yield* makeTempDir("iskra-case-aliases-");
    ...
    it.effect("uses explicit provider instance homes instead of overridden legacy homes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("iskra-claude-legacy-");
        const codexHomePath = yield* makeTempDir("iskra-codex-legacy-");
        const claudeInstanceHome = yield* makeTempDir("iskra-claude-instance-");
        const codexInstanceHome = yield* makeTempDir("iskra-codex-instance-");
        const legacyWorkspace = yield* makeTempDir("iskra-workspace-legacy-");
        const claudeWorkspace = yield* makeTempDir("iskra-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("iskra-workspace-codex-");
    ...
    it.effect("scans each distinct home across multiple instances once", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("iskra-claude-home-");
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const otherCodexHome = yield* makeTempDir("iskra-codex-other-");
        const workspace = yield* makeTempDir("iskra-workspace-");
        const otherWorkspace = yield* makeTempDir("iskra-workspace-other-");
    ...
    it.effect("honors provider instance home directory environment variables", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("iskra-claude-legacy-");
        const codexHomePath = yield* makeTempDir("iskra-codex-legacy-");
        const claudeEnvironmentHome = yield* makeTempDir("iskra-claude-env-");
        const codexEnvironmentHome = yield* makeTempDir("iskra-codex-env-");
        const claudeWorkspace = yield* makeTempDir("iskra-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("iskra-workspace-codex-");
    ...
    it.effect("does not scan provider instances disabled by the envelope or config", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("iskra-claude-home-");
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const envelopeDisabledHome = yield* makeTempDir("iskra-codex-disabled-envelope-");
        const configDisabledHome = yield* makeTempDir("iskra-codex-disabled-config-");
        const envelopeWorkspace = yield* makeTempDir("iskra-workspace-disabled-envelope-");
        const configWorkspace = yield* makeTempDir("iskra-workspace-disabled-config-");
    ...
    it.effect("excludes the home directory, temporary root, and Iskra data directory", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("iskra-claude-home-");
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const configBaseDir = yield* makeTempDir("iskra-scanner-base-");
        const workspace = yield* makeTempDir("iskra-workspace-");
    ...
    it.effect("excludes T3-managed worktree sandboxes", () =>
      Effect.gen(function* () {
        ...
        const worktreeCwd = path.join(claudeHomePath, ".t3", "worktrees", "iskra", "wt-1");
    ...
    it.effect("excludes Codex scratch directories and Downloads", () =>
      Effect.gen(function* () {
        ...
        const runId = path.basename(yield* makeTempDir("iskra-scanner-test-"));
        ...
        const keep = yield* makeTempDir("iskra-workspace-keep-");
    ...
    it.effect("skips linked git worktrees and reports the origin of real checkouts", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("iskra-claude-home-");
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const repo = yield* makeTempDir("iskra-workspace-repo-");
        const worktree = yield* makeTempDir("iskra-workspace-worktree-");
        const plain = yield* makeTempDir("iskra-workspace-plain-");
        const noRemote = yield* makeTempDir("iskra-workspace-noremote-");
        const submodule = yield* makeTempDir("iskra-workspace-submodule-");

        yield* fileSystem.makeDirectory(path.join(repo, ".git"));
        yield* fileSystem.writeFileString(
          path.join(repo, ".git", "config"),
          '[core]\n\tbare = false\n[remote "origin"]\n\turl = git@github.com:boroworx/iskra.git\n\tfetch = +refs/heads/*:refs/remotes/origin/*\n',
        );
        yield* fileSystem.writeFileString(
        ...
        expect(
          ...
          {
            path: repo,
            git: { remoteKey: "github.com/boroworx/iskra", repository: "boroworx/iskra" },
          },
        ]);
      }),
    );
    ...
    it.effect("excludes sandboxes under the configured worktrees dir without .t3 in the path", () =>
      Effect.gen(function* () {
        ...
        const worktreeCwd = path.join(configBaseDir, "worktrees", "iskra", "wt-2");
        yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
        yield* writeTranscript({
          filePath: path.join(claudeHomePath, "projects", "-slug", "a.jsonl"),
          contents: claudeSessionLine(worktreeCwd),
    ...
    it.effect("excludes sandboxes reached through a symlink into the worktrees dir", () =>
      Effect.gen(function* () {
        ...
        const worktreeCwd = path.join(configBaseDir, "worktrees", "iskra", "wt-3");
        yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
    ...
    it.effect.each([64, 65])("shares metadata bytes across homes for %s one-MiB files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("iskra-metadata-home-");
        const secondHome = yield* makeTempDir("iskra-metadata-second-");
        const codexHomePath = yield* makeTempDir("iskra-metadata-codex-");
        const firstWorkspace = yield* makeTempDir("iskra-metadata-first-project-");
        const secondWorkspace = yield* makeTempDir("iskra-metadata-second-project-");
        const directories = [
    ...
    it.effect.each([50, 51])("bounds metadata open/read calls for %s short-read files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("iskra-short-metadata-home-");
        const codexHomePath = yield* makeTempDir("iskra-short-metadata-codex-");
        const workspace = yield* makeTempDir("iskra-short-metadata-project-");
    ...
    it.effect("bounds malformed metadata records without excluding another account", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("iskra-record-metadata-home-");
        const secondHome = yield* makeTempDir("iskra-record-metadata-second-");
        const codexHomePath = yield* makeTempDir("iskra-record-metadata-codex-");
        const workspace = yield* makeTempDir("iskra-record-metadata-project-");
        const directory = path.join(claudeHomePath, "projects", "p");
        const template = path.join(directory, "template.jsonl");
        yield* writeTranscript({
          filePath: template,
    ...
    it.effect.each([19_999, 20_000])(
      ...
      (count) =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const fileSystem = yield* FileSystem.FileSystem;
          const claudeHomePath = yield* makeTempDir("iskra-directory-budget-home-");
          const codexHomePath = yield* makeTempDir("iskra-directory-budget-codex-");
    ...
    it.effect("returns an empty result when neither home directory exists", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const root = yield* makeTempDir("iskra-missing-homes-");
  ...
  describe("recentThreads", () => {
    it.effect.each([false, true])(
      ...
      (overflow) =>
        Effect.gen(function* () {
          ...
          const claudeHomePath = yield* makeTempDir("iskra-record-limit-claude-");
          const codexHomePath = yield* makeTempDir("iskra-record-limit-codex-");
          const workspace = yield* makeTempDir("iskra-record-limit-project-");
    ...
    it.effect("keeps the provider instance that owns a custom session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("iskra-claude-home-");
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const customHome = yield* makeTempDir("iskra-codex-custom-");
        const workspace = yield* makeTempDir("iskra-workspace-");
    ...
    it.effect("streams large transcripts across providers without hiding projects", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("iskra-budget-claude-");
        const codexHomePath = yield* makeTempDir("iskra-budget-codex-");
        const workspace = yield* makeTempDir("iskra-budget-workspace-");
    ...
    it.effect("skips excessive records without blocking an older valid transcript", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("iskra-record-budget-claude-");
        const codexHomePath = yield* makeTempDir("iskra-record-budget-codex-");
        const workspace = yield* makeTempDir("iskra-record-budget-workspace-");
        ...
        it.effect.skipIf(replacement === "symlink alias" && !symlinksSupported)(
          ...
          () =>
            Effect.gen(function* () {
              ...
              const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
              yield* TestClock.setTime(nowMs);
              const fixture = yield* makeTempDir("iskra-replaced-cwd-");
    ...
    it.effect("checks file identity and provider before skipping completed history", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("iskra-completed-claude-");
        const codexHomePath = yield* makeTempDir("iskra-completed-codex-");
        const workspace = yield* makeTempDir("iskra-completed-workspace-");
    ...
    it.effect("does not reopen a transcript that becomes a non-file after discovery", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const workspace = yield* makeTempDir("iskra-workspace-");
        const nonFilePath = yield* makeTempDir("iskra-non-file-");
    ...
    it.effect("does not import sessions from a T3-managed worktree", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const configBaseDir = yield* makeTempDir("iskra-scanner-base-");
        const workspace = path.join(configBaseDir, "worktrees", "iskra", "managed-worktree");
    ...
    it.effect("uses one deterministic provider instance for a shared session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("iskra-claude-home-");
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const sharedHome = yield* makeTempDir("iskra-codex-shared-");
        const workspace = yield* makeTempDir("iskra-workspace-");
    ...
    it.effect("keeps a second account when the first has 5000 newer files", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("iskra-claude-home-");
        const codexHomePath = yield* makeTempDir("iskra-codex-home-");
        const oldWorkspace = yield* makeTempDir("iskra-workspace-old-");
        const recentWorkspace = yield* makeTempDir("iskra-workspace-recent-");
        const recentHome = yield* makeTempDir("iskra-claude-recent-home-");
