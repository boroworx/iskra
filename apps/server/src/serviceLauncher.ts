...
const runtimePaths = (baseDir: string, version: string) => {
  ...
  // oxlint-disable-next-line iskra/no-global-process-runtime -- Standalone launcher has no Effect runtime.
...
export class Launcher {
  ...
  async #recover(): Promise<void> {
    ...
    // handoff release its tunnel. A restart deferred by `iskra update` is done
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected @iskra/cli@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
