...
import { GitManagerError } from "@iskra/contracts";
...
const remoteStatusWithPr: VcsStatusRemoteResult = {
  ...
  pr: {
    ...
    url: "https://github.com/boroworx/iskra/pull/2978",
    baseRef: "main",
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const realDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "iskra-vcs-auto-pull-real-",
        });
        const linkParent = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "iskra-vcs-auto-pull-link-",
        });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const realDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "iskra-vcs-status-real-",
        });
        const linkParent = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "iskra-vcs-status-link-",
        });
