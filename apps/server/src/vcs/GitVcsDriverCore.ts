...
import { dedupeRemoteBranchesWithLocalMatches, normalizeGitRemoteUrl } from "@iskra/shared/git";
import { compactTraceAttributes } from "@iskra/shared/observability";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `iskra-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const readUnifiedWorkingTreeReviewDiff = Effect.fn("readUnifiedWorkingTreeReviewDiff")(function* (
    ...
    const tempIndexPath = yield* fileSystem.makeTempFileScoped({
      prefix: `iskra-review-index-${process.pid}-`,
    });
    ...
    Effect.fn("refreshCheckedOutBranch")(function* (input) {
      ...
      if (!isAncestor) {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["update-ref", "refs/iskra/pre-refresh", headCommit],
