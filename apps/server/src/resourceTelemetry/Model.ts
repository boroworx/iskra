...
export interface TelemetryCounters {
  ...
  readonly allIskra: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allIskra: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allIskra = input.counters.allIskra;
  for (const processDelta of input.deltas) {
    ...
    allIskra = incrementCounters(allIskra, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allIskra = incrementCounters(allIskra, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allIskra = incrementCounters(allIskra, { processExits: 1 });
  }

  return { backend, electron, monitor, allIskra };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allIskra: aggregate(ordered, counters.allIskra),
    },
    deltas: processDeltas,
  };
}
