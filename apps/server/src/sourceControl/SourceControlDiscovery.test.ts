...
import { VcsProcessSpawnError } from "@iskra/contracts";
    ...
    Layer.provide(
      Layer.mergeAll(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "iskra-source-control-registry-test-",
...
it.effect("reports implemented tools separately from locally available executables", () => {
  ...
  const testLayer = SourceControlDiscovery.layer.pipe(
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "iskra-source-control-discovery-",
      }),
    ),
...
`),
  ...
  const testLayer = SourceControlDiscovery.layer.pipe(
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "iskra-source-control-auth-discovery-",
      }),
    ),
    Layer.provide(Layer.mock(VcsProcess.VcsProcess)(processMock)),
    Layer.provide(
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-fj-checkout-" });
