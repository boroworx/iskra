...
import { sanitizeBranchFragment } from "@iskra/shared/git";
import {
  detectSourceControlProviderFromRemoteUrl,
  isSshRemoteUrl,
} from "@iskra/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("ISKRA_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("ISKRA_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("ISKRA_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("ISKRA_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("@iskra/cli/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `iskra/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set ISKRA_BITBUCKET_EMAIL and ISKRA_BITBUCKET_API_TOKEN, or ISKRA_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
