    ...
    const provider = yield* makeProvider({
      getPullRequest: () =>
        Effect.succeed({
          ...
          headRepositoryNameWithOwner: "fork/iskra",
...
it.effect("adds safe request context while retaining GitHub CLI causes", () =>
  Effect.gen(function* () {
    ...
    const error = yield* provider
      .getChangeRequest({
        cwd: "/repo",
        reference: "https://user:secret@github.com/boroworx/iskra/pull/42?token=secret#diff",
      })
      .pipe(Effect.flip);
    ...
    assert.deepStrictEqual(
      ...
      },
      {
        ...
        command: "gh",
        cwd: "/repo",
        reference: "https://github.com/boroworx/iskra/pull/42",
