...
import { SpawnExecutableResolution } from "@iskra/shared/shell";
...
it.effect("launches an installed editor with platform-safe arguments", () =>
  Effect.gen(function* () {
    ...
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "iskra-editors-" });
...
// oxlint-disable-next-line iskra/no-global-process-runtime -- the skip decision needs the real host platform, outside any Effect runtime.
it.skipIf(process.platform !== "win32")(
  "delivers the raw /select switch for spaced paths through real PowerShell",
  { timeout: 60_000 },
  async () => {
    const tempDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "iskra-reveal-smoke-"));
    try {
  ...
  () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.gen(function* () {
        ...
        yield* launcher.launchEditor({
          editor: "file-manager",
          cwd: "/home/iskra/workspace/media/clip.mp4",
          reveal: true,
        });
        return { kind, editors };
      }).pipe(
...
it.effect.skipIf(windowsHost)(
  ...
  () =>
    Effect.gen(function* () {
      ...
      assert.ok(launch);
      assert.deepEqual(launch.args, ["/home/iskra/workspace/media"]);
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* launcher.launchEditor({
          editor: "file-manager",
          cwd: '/home/iskra/work "quoted"/clip.mp4',
          reveal: true,
        });
      }).pipe(
        Effect.provide(
      ...
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\iskra-editor-discovery-cache-test",
              PATHEXT: ".COM;.EXE;.BAT;.CMD",
            },
          }),
        ),
    ...
    Effect.provide(
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\iskra-editor-discovery-interrupt-test",
              PATHEXT: ".COM;.EXE;.BAT;.CMD",
            },
          }),
        ),
