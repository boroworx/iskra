...
const testLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "iskra-cursor-driver-copy-command-",
}).pipe(
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-cursor-driver-" });
    ...
    Effect.gen(function* () {
      const instance = yield* CursorDriver.create({
        ...
        config: {
          ...
          binaryPath: NodePath.join(NodeOS.tmpdir(), "iskra-cursor-missing", "cursor-agent"),
        },
      });
