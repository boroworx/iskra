...
import { AntigravitySettings, ProviderDriverKind, ProviderSetupError } from "@iskra/contracts";
    ...
    Effect.gen(function* () {
      ...
      const makeDisposableRuntime = Effect.fn("AntigravityDriver.makeDisposableRuntime")(function* (
        ...
        const cwd = yield* fileSystem
          .makeTempDirectoryScoped({ prefix: "iskra-antigravity-setup-" })
          .pipe(
            Effect.mapError(
              () =>
                new ProviderSetupError({
        ...
        const runtime = yield* makeRuntime({
          cwd,
          clientInfo: { name: "iskra-provider-setup", version: "0.0.0" },
          mcpServers: [],
      ...
      const textGeneration = yield* makeAntigravityTextGeneration({
        ...
        makeRuntime: (cwd) =>
          makeRuntime({
            cwd,
            clientInfo: { name: "iskra-text", version: "0.0.0" },
            mcpServers: [],
          }),
      });
