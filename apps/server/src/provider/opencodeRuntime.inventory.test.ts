...
it.layer(testLayer)("OpenCodeRuntime inventory", (it) => {
  ...
  it.effect("drops oversized CLI skill output without losing the model inventory", () =>
    Effect.gen(function* () {
      ...
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-opencode-inventory-" });
      ...
      yield* fs.writeFileString(
        ...
        [
          ...
          isWindows
            ? '"%ISKRA_TEST_NODE_BINARY%" "%ISKRA_TEST_OPENCODE_SCRIPT%" %*'
            : 'exec "$ISKRA_TEST_NODE_BINARY" "$ISKRA_TEST_OPENCODE_SCRIPT" "$@"',
          "",
        ].join("\n"),
      );
      if (!isWindows) {
      ...
      const inventory = yield* runtime.loadInventoryFromCli({
        ...
        environment: {
          ...hostEnvironment,
          ISKRA_TEST_NODE_BINARY: executablePath,
          ISKRA_TEST_OPENCODE_SCRIPT: scriptPath,
        },
      });
