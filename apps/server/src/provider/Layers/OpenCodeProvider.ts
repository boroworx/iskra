...
import { compareSemverVersions } from "@iskra/shared/semver";
import {
  buildServerProvider,
        ...
        probe: {
          ...
          message:
            openCodeSettings.serverUrl.trim().length > 0
              ? "OpenCode is disabled in Iskra settings. A server URL is configured."
              : "OpenCode is disabled in Iskra settings.",
        },
      });
    }
...
> {
  ...
  if (!isExternalServer) {
    ...
    if (!version) {
      return fallback(
        new Error(
          `Unable to determine OpenCode version from \`opencode --version\` output. Iskra requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
        ),
        null,
      );
    }
