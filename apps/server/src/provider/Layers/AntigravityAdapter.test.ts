...
const layer = ServerConfig.layerTest(process.cwd(), {
  prefix: "iskra-antigravity-adapter-test-",
...
it.layer(layer)("AntigravityAdapter", (it) => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "iskra-antigravity-transport-",
        });
          ...
          makeRuntime: (input) =>
            makeAntigravityAcpRuntime({
              ...
              spawn: {
                ...
                env: {
                  ...process.env,
                  ISKRA_ACP_ANTIGRAVITY: "1",
                  ISKRA_ACP_REQUEST_LOG_PATH: requestLog,
                },
                extendEnv: false,
              },
  ...
  it.effect("serves client file reads and writes only inside the session roots", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-agy-fs-" });
      const outside = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-agy-outside-" });
