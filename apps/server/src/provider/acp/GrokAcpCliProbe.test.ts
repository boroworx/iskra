 ...
 * Enable with: ISKRA_GROK_ACP_PROBE=1 vp test run GrokAcpCliProbe
 * Set ISKRA_GROK_LIVE_TURN=1 to also send a small prompt to the real model.
 *
...
const makeProbeRuntime = Effect.gen(function* () {
  ...
  return yield* makeGrokAcpRuntime({
    ...
    childProcessSpawner,
    cwd: process.cwd(),
    clientInfo: { name: "iskra-grok-probe", version: "0.0.0" },
  });
});

describe.runIf(process.env.ISKRA_GROK_ACP_PROBE === "1")("Grok ACP CLI probe", () => {
  ...
  it.effect.skipIf(process.env.ISKRA_GROK_LIVE_TURN !== "1")(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const result = yield* runtime.prompt({
          prompt: [{ type: "text", text: "Reply exactly GROK_ISKRA_OK. Do not use any tools." }],
        });
        yield* runtime.drainEvents;
        expect(result.stopReason).toBe("end_turn");
        expect(chunks.join("")).toContain("GROK_ISKRA_OK");
