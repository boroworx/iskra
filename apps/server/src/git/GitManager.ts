...
import { hasProjectSettingsOverrides, resolveProjectSettings } from "@iskra/shared/projectSettings";
...
import { GitManagerError, GitPullRequestMaterializationError } from "@iskra/contracts";
...
import type { GitManagerServiceError } from "@iskra/contracts";
import * as GitVcsDriver from "../vcs/GitVcsDriver.ts";
import * as SourceControlProviderRegistry from "../sourceControl/SourceControlProviderRegistry.ts";
import { detectPrTemplate } from "../sourceControl/PrTemplateDetection.ts";
import type { ChangeRequest } from "@iskra/contracts";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `iskra/pr-${pullRequest.number}/${suffix}`;
}
...
export const make = Effect.gen(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(
      tempDir,
      `iskra-pr-body-${process.pid}-${yield* randomUUIDv4(cwd)}.md`,
    );
