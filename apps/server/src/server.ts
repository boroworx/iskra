import { EnvironmentHttpApi, ProviderDriverKind, type RepositoryIdentity } from "@iskra/contracts";
...
import { disableTailscaleServe, ensureTailscaleServe } from "@iskra/tailscale";
...
// Iskra's primary transport is long-lived WebSocket RPC, whose Effect scope finalizer
...
const ReactorLayerLive = Layer.empty.pipe(
  ...
  Layer.provideMerge(
    LinearSyncReactor.layer.pipe(
      Layer.provide(LinearClient.layer.pipe(Layer.provide(ServerSecretStore.layer))),
    ),
  ),
  Layer.provideMerge(
        ...
        yield* forkParked(
          Effect.gen(function* () {
            ...
            yield* reconcileDesiredCloudLink(`http://127.0.0.1:${address.port}`).pipe(
              ...
              Effect.tap(() => Effect.logInfo("Iskra Connect desired link reconciled on startup")),
              Effect.catch((cause) =>
                Effect.logWarning("Failed to reconcile Iskra Connect desired link on startup", {
