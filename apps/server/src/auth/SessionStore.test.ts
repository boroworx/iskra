...
it.layer(NodeServices.layer)("SessionStore.layer", (it) => {
  ...
  it.effect("keeps reusable dev auth local across disk-backed stores and restarts", () =>
    Effect.gen(function* () {
      ...
      const baseA = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-dev-auth-a-" });
      const baseB = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-dev-auth-b-" });
  ...
  it.effect("invalidates old dev credentials and tickets after rotation or removal", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-dev-auth-rotation-" });
