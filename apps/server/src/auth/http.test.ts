...
import { EnvironmentHttpApi } from "@iskra/contracts";
...
).pipe(Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "iskra-auth-http-test-" })));
    ...
    return yield* Effect.acquireUseRelease(
      ...
      ([environmentA, environmentB]) =>
        Effect.tryPromise(async () => {
          ...
          const devCookie = devCookies.find((cookie) => cookie.startsWith("iskra_dev_session_"));
          expect(devCookie).toContain("HttpOnly");
          expect(devCookie).toContain(`=${DEV_TOKEN};`);
          expect(devCookies).toContainEqual(
            expect.stringMatching(/^iskra_session_[^=]*=;.*Max-Age=0/),
          );
          ...
          expect(restrictedCookies[0]).toMatch(/^iskra_session_/);
          expect(restrictedCookies[0]).not.toContain("iskra_dev_session_");
        }),
      ([environmentA, environmentB]) =>
