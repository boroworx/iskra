...
const version = "1.2.3";
const archiveName = `iskra-${version}-linux-x64.tar.gz`;
  ...
  ProcessRunner.ProcessRunner.of({
    run: (input) =>
      Effect.gen(function* () {
        ...
        yield* fs.writeFileString(path.join(stagingDir, "iskra"), "#!/bin/sh\n").pipe(Effect.orDie);
        return {
          stdout: "",
          stderr: "",
...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("installs the verified release archive as the runtime executable", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-pinned-archive-" });
      ...
      assert.equal(paths.entryPath, path.join(paths.versionDir, "iskra"));
      ...
      assert.isFalse(yield* fs.exists(path.join(paths.versionDir, "iskra-runtime-archive")));
      if ((yield* HostProcessPlatform) !== "win32") {
        ...
        const legacyStart = yield* runner.run({
          ...
          args: [path.join(paths.versionDir, "node_modules/@iskra/cli/dist/bin.mjs"), "serve"],
        });
  ...
  it.effect("refuses an archive whose checksum does not match the release", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-pinned-archive-bad-" });
      const commands: string[] = [];
      ...
      assert.equal(error.step, "verifying the iskra release archive checksum");
  ...
  it.effect("validates a staging tree before atomically publishing it", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-pinned-runtime-test-" });
  ...
  it.effect("replaces an incomplete pinned runtime", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-pinned-runtime-repair-" });
      const finalPaths = pinnedRuntimePaths(path, baseDir, version, "linux");
  ...
  it.effect("backfills a cached archive without downloading or replacing it", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-pinned-legacy-cache-" });
      const cached = pinnedRuntimePaths(path, baseDir, version, "linux");
      const legacyEntry = path.join(cached.versionDir, "node_modules/@iskra/cli/dist/bin.mjs");
  ...
  it.effect("removes staging when installation is interrupted", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-pinned-runtime-interrupt-" });
