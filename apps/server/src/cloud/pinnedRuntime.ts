...
/**
 * A pinned runtime is an exact iskra release archive unpacked into
 ...
 * Node or npm on the machine; the only npm involvement in Iskra is the `iskra`
 * package for people who prefer `npx @iskra/cli` or `npm install -g @iskra/cli`, and even a
 * CLI installed that way pins an archive when it sets up the service.
 */
const PINNED_RUNTIME_DIR = "runtime";
const PINNED_RUNTIME_INSTALL_TIMEOUT = Duration.minutes(10);
const PINNED_RUNTIME_ARCHIVE_FILE = "iskra-runtime-archive";
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, platform === "win32" ? "iskra.exe" : "iskra"),
...
/**
 * Installs the iskra release archive for `version` into the pinned runtime
...
const installFromArchive = Effect.fn("cloud.pinned_runtime.install_archive")(function* (
  ...
  if (platformKey === undefined) {
    return yield* new PinnedRuntimeInstallError({
      step: `selecting a iskra release archive for ${input.platform}-${input.arch}`,
    });
  }
  ...
  const checksums = parseChecksums(
    new TextDecoder().decode(
      yield* fetchReleaseAsset(
        ...
        "downloading the iskra release checksums",
      ),
    ),
  );
  ...
  if (expected === undefined) {
    return yield* new PinnedRuntimeInstallError({
      step: `finding ${fileName} in the iskra release checksums`,
    });
  }
  const archive = yield* fetchReleaseAsset(
    httpClient,
    `${baseUrl}/${fileName}`,
    "downloading the iskra release archive",
  );
  const digest = yield* Effect.tryPromise({
    try: () => crypto.subtle.digest("SHA-256", archive),
    catch: (cause) =>
      new PinnedRuntimeInstallError({ step: "verifying the iskra release archive", cause }),
  });
  if (Encoding.encodeHex(new Uint8Array(digest)) !== expected) {
    return yield* new PinnedRuntimeInstallError({
      step: "verifying the iskra release archive checksum",
    });
  }
  ...
  yield* fs
    ...
    .pipe(
      Effect.mapError(
        (cause) => new PinnedRuntimeInstallError({ step: "writing the iskra release archive", cause }),
      ),
    );
  const extractStep = "extracting the iskra release archive";
  // The archive wraps everything in one directory named after its stem;
  // strip it so the executable lands at <versionDir>/iskra.
  yield* input.runner
    .run({
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const ensureLegacyEntry = Effect.fn("cloud.pinned_runtime.ensure_legacy_entry")(
    function* (versionDir: string) {
      const legacyDir = input.path.join(versionDir, "node_modules", "@iskra", "cli", "dist");
