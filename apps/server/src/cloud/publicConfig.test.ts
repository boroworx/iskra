...
it.effect("prefers a runtime relay URL override over the statically injected value", () =>
  Effect.gen(function* () {
    const relayUrl = yield* makeRelayUrlConfig("https://embedded.example.test").pipe(
      provideEnv({ ISKRA_RELAY_URL: "https://runtime.example.test///" }),
    );
...
it.effect("rejects an insecure runtime relay URL override", () =>
  makeRelayUrlConfig("https://embedded.example.test").pipe(
    provideEnv({ ISKRA_RELAY_URL: "http://runtime.example.test" }),
    Effect.flip,
  ),
);
  ...
  Effect.gen(function* () {
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ ISKRA_HOSTED_APP_URL: "https://nightly.app.iskra.sh" }),
      ),
      "https://nightly.app.iskra.sh",
    );
    assert.equal(
      yield* hostedAppUrlConfig.pipe(provideEnv({ ISKRA_HOSTED_APP_URL: "http://localhost:5733" })),
      "http://localhost:5733",
    );
  }),
);
...
it.effect("rejects malformed or insecure hosted app URLs", () =>
  Effect.gen(function* () {
    for (const value of [
      "app.iskra.sh",
      "http://app.iskra.sh",
      "https://app.iskra.sh/nested",
      "https://app.iskra.sh?alias=true",
    ]) {
      const result = yield* hostedAppUrlConfig.pipe(
        provideEnv({ ISKRA_HOSTED_APP_URL: value }),
        Effect.result,
      );
...
it.effect("prefers runtime Clerk OAuth config overrides over statically injected values", () =>
  Effect.gen(function* () {
    const config = yield* makeCloudCliOAuthConfig({
      ...
      provideEnv({
        ISKRA_CLERK_PUBLISHABLE_KEY: "pk_test_cnVudGltZS5leGFtcGxlLnRlc3Qk",
        ISKRA_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_client_runtime",
      }),
    );
...
it("resolves relay client tracing from runtime config with build-time fallback", () => {
  ...
  assert.deepEqual(
    resolveRelayClientTracingConfig(
      {
        ISKRA_RELAY_CLIENT_OTLP_TRACES_URL: "https://runtime.example.test/v1/traces",
        ISKRA_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        ISKRA_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
    ),
    {
  ...
  assert.equal(
    resolveRelayClientTracingConfig(
      {
        ISKRA_RELAY_CLIENT_OTLP_TRACES_URL: "http://insecure.example.test/v1/traces",
        ISKRA_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        ISKRA_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
