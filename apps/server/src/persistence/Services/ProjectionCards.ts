...
export const ProjectionCard = Schema.Struct({
  ...
  // Always set on rows (migration 079 backfilled it); optional only because the card type is.
  origin: Schema.optional(CardOrigin),
  plan: Schema.NullOr(CardPlan),
  migration: Schema.NullOr(CardMigration),
  planKey: Schema.NullOr(Schema.String),
  slice: Schema.NullOr(Schema.Number),
  heldByCheckpoint: Schema.Boolean,
  unattended: Schema.Boolean,
  outcome: Schema.NullOr(CardOutcome),
  landedSha: Schema.NullOr(Schema.String),
  revertsCardId: Schema.NullOr(CardId),
...
export const ProjectionCardDbRow = ProjectionCard.mapFields(
  Struct.assign({
    ...
    origin: Schema.optional(Schema.fromJsonString(CardOrigin)),
    plan: Schema.fromJsonString(Schema.NullOr(CardPlan)),
    migration: Schema.fromJsonString(Schema.NullOr(CardMigration)),
    heldByCheckpoint: Schema.fromJsonString(Schema.Boolean),
    unattended: Schema.fromJsonString(Schema.Boolean),
    outcome: Schema.fromJsonString(Schema.NullOr(CardOutcome)),
  }),
);
...
export const PROJECTION_CARD_COLUMNS = `
  ...
  COALESCE(origin_json, '{"kind":"human","id":null}') AS "origin",
  COALESCE(plan_json, 'null') AS "plan",
  COALESCE(migration_json, 'null') AS "migration",
  plan_key AS "planKey",
  slice,
  CASE held_by_checkpoint WHEN 1 THEN 'true' ELSE 'false' END AS "heldByCheckpoint",
  CASE unattended WHEN 1 THEN 'true' ELSE 'false' END AS "unattended",
  COALESCE(outcome_json, 'null') AS "outcome",
  landed_sha AS "landedSha",
  reverts_card_id AS "revertsCardId",
