...
it.layer(NodeServices.layer)("iskra update launcher", (it) => {
  it.effect("repoints a symlink that lives in a runtime versions tree", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-update-" });
      const oldExe = path.join(root, "runtime/versions/1.0.0/iskra");
      const newExe = path.join(root, "runtime/versions/2.0.0/iskra");
      const launcher = path.join(root, "bin/iskra");
  ...
  it.effect("leaves a plain copy or a foreign symlink alone", () =>
    Effect.gen(function* () {
      ...
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-update-" });
      const newExe = path.join(root, "runtime/versions/2.0.0/iskra");
      const copy = path.join(root, "copy/iskra");
      const foreign = path.join(root, "foreign/iskra");
      const elsewhere = path.join(root, "elsewhere/iskra");
      // Another install's versions tree: same shape, different home.
      const otherHome = path.join(root, "other/runtime/versions/1.0.0/iskra");
      const otherLauncher = path.join(root, "other/bin/iskra");
  ...
  it.effect("finds the launcher a bare command name resolved to on PATH", () =>
    Effect.gen(function* () {
      ...
      const bare = yield* resolveLauncherPath.pipe(
        Effect.provideService(HostProcessInvokedAs, "iskra"),
      ...
      const relative = yield* resolveLauncherPath.pipe(
        Effect.provideService(HostProcessInvokedAs, "./bin/iskra"),
