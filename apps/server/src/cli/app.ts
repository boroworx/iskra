...
export class DesktopAppSshUnsupportedError extends Schema.TaggedError<DesktopAppSshUnsupportedError>()(
  ...
  override get message(): string {
    return "`iskra app` only controls a desktop app on the same machine. It cannot run over SSH.";
  }
}
...
export class DesktopAppPlatformUnsupportedError extends Schema.TaggedError<DesktopAppPlatformUnsupportedError>()(
  ...
  override get message(): string {
    return `\`iskra app\` is not supported on ${this.platform}.`;
  }
}
...
export class DesktopAppUnreachableError extends Schema.TaggedError<DesktopAppUnreachableError>()(
  ...
  override get message(): string {
    return "Could not reach the Iskra desktop app. Start or update the desktop app on this machine, then run `iskra app` again. A running Iskra server is not enough.";
  }
}
...
export class DesktopAppRequestFailedError extends Schema.TaggedError<DesktopAppRequestFailedError>()(
  ...
  override get message(): string {
    return `Iskra could not open ${this.workspaceRoot} (${this.code}).`;
  }
}
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
  ...
  const configuredBaseDir = Option.getOrUndefined(flags.baseDir) ?? environment.iskraHome;
  const baseDir = yield* resolveBaseDir(configuredBaseDir);
  const allowDevFallback = Option.isNone(flags.baseDir) && !environment.iskraHome?.trim();
  ...
  yield* Console.log(`Opened ${workspaceRoot} in Iskra.`);
});
...
export const appCommand = Command.make("app", {
  ...
  Command.withDescription("Open a project in the running Iskra desktop app."),
