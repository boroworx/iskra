...
export function formatServiceStatus(
  ...
  if (
    ...
    return [
      ...
      `  Logs: ${status.logPath}`,
      ...problems,
      `  Next: Use \`npx @iskra/cli@${installedVersion} service update\` to repair it, or pass \`--allow-downgrade\` explicitly.`,
    ].join("\n");
  }
  return [
    "Iskra service",
    ...
    `  Logs: ${status.logPath}`,
    ...problems,
    ...(status.current ? [] : [`  Next: Run \`npx @iskra/cli@${cliVersion} service update\`.`]),
  ].join("\n");
}
...
const serviceUpdateCommand = Command.make("update", serviceReconcileFlags).pipe(
  Command.withDescription(
    "Update or repair the background service using this CLI version. Use `npx @iskra/cli@latest service update` for the latest release.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
