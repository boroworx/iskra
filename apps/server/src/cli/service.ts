...
export function formatServiceStatus(
  ...
  if (!status.supported) {
    return "Iskra service\n  Status: unavailable on this machine\n  Supported on: Linux with systemd, macOS with launchd";
  }
  if (!status.installed) {
    return "Iskra service\n  Status: not installed\n  Next: Run `iskra service install`.";
  }
  ...
  if (
    ...
    return [
      "Iskra service",
      `  Status: installed · @iskra/cli@${installedVersion} (newer than this @iskra/cli@${cliVersion} CLI)`,
      `  Unit: ${status.unitPath}`,
      `  Logs: ${status.logPath}`,
      ...problems,
      `  Next: Run \`iskra update ${installedVersion}\` to match it, or pass \`--allow-downgrade\` to \`iskra service install\` explicitly.`,
    ].join("\n");
  }
  return [
    "Iskra service",
    `  Status: ${status.current ? `installed · @iskra/cli@${installedVersion}` : "needs an update or repair"}`,
    `  Unit: ${status.unitPath}`,
    `  Logs: ${status.logPath}`,
    ...problems,
    ...(status.current ? [] : ["  Next: Run `iskra service install` to repair it."]),
  ].join("\n");
}
...
const serviceInstallCommand = Command.make("install", serviceReconcileFlags).pipe(
  Command.withDescription("Install Iskra as a background service for this user."),
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `Iskra service is already installed with @iskra/cli@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Iskra service with @iskra/cli@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
...
// Kept one release for muscle memory and old docs. It did what `iskra service
// install` does; the way to move to a newer release is `iskra update`.
const serviceUpdateCommand = Command.make("update", serviceReconcileFlags).pipe(
  Command.withDescription("Deprecated. Run `iskra update` to move to a newer release."),
  Command.unlisted,
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log(
          "`iskra service update` is deprecated: run `iskra update` to move to a newer release, or `iskra service install` to repair the service. Repairing now.",
        );
        const result = yield* reconcileService({ allowDowngrade: flags.allowDowngrade });
        if (!result.changed) {
          yield* Console.log(`Iskra service is already using @iskra/cli@${packageJson.version}.`);
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Iskra service with @iskra/cli@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
...
const serviceRestartCommand = Command.make("restart", projectLocationFlags).pipe(
  Command.withDescription(
    "Restart the background service. Picks up a version installed by `iskra update` that was not restarted at the time.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          restarted
            ? `Restarted the Iskra service${status.installedVersion === undefined ? "" : ` on iskra@${status.installedVersion}`}.`
            : "Iskra service is not installed.",
        );
      }),
    ),
  ),
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the Iskra background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed ? "Removed the Iskra service." : "Iskra service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the Iskra background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log("Iskra is already set up to run in the background on this machine.");
    return true;
  }
  ...
  if (
    ...
    yield* Console.log(
      `A newer @iskra/cli@${status.installedVersion} background service is installed. Leaving it unchanged.`,
    );
  ...
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed Iskra service needs an update or repair. Update it now?"
        : platform === "darwin"
          ? "Run Iskra in the background whenever you log in to this Mac? " +
            "It stays reachable through Iskra Connect while you are logged in."
          : "Run Iskra in the background whenever this machine boots? " +
            "It stays reachable through Iskra Connect even after you log out.",
      initial: true,
    }),
  );
  if (!wanted) {
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the Iskra background service."),
