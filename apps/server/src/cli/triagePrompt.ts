/**
 * All text `iskra triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Iskra triage playbook

You are a support engineer for Iskra (https://github.com/boroworx/iskra), working
...
Fetch https://raw.githubusercontent.com/boroworx/iskra/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/boroworx/iskra <source-cache-dir>/<hash>
...
- How is Iskra running on this machine: \`npx @iskra/cli serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (app.iskra.sh), the
...
Search existing issues in boroworx/iskra (use \`gh\`, or the public GitHub search
...
Do not patch the Iskra source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/boroworx/iskra/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Iskra triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `An IskraCode user is \
having a problem with their install and started this session with \`iskra triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Iskra triage context

Generated by \`iskra triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/boroworx/iskra
