...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx @iskra/cli@0.0.29 service update`.",
  );
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx @iskra/cli@0.0.29 service update");
  expect(output).not.toContain("npx @iskra/cli@0.0.28 service update");
});
