...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `iskra service install` to repair it.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("Run `iskra service install` to repair it.");
...
it("reports a newer installed service and tells the CLI to catch up to it", () => {
  ...
  assert.include(output, "Run `iskra update 0.0.32-nightly.1` to match it");
  ...
  const service = BootService.BootService.of({
    ...
    install: (options) =>
      Effect.sync(() => {
        ...
        return {
          program: ["/test/iskra/runtime/versions/1.0.0/iskra", "__service-launcher"],
          baseDir: "/test/iskra",
