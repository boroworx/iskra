/**
 * `iskra triage` - hand a misbehaving install to the user's own coding agent.
 *
...
import { isCommandAvailable, resolveSpawnCommand } from "@iskra/shared/shell";
...
export const triageCommand = Command.make("triage", {
  ...
  Command.withDescription(
    "Investigate an IskraCode problem on this machine with claude or codex, and help file a good issue.",
  ),
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // --base-dir wins; ISKRA_HOME is its documented env equivalent (same
      // precedence as `iskra pair`).
