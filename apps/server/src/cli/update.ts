  ...
  for (let page = 1; page <= RELEASE_INDEX_MAX_PAGES; page += 1) {
    const body = yield* httpClient
      ...
      .pipe(
        ...
        Effect.mapError(() => new CliUpdateError({ reason: "Could not list iskra releases." })),
        Effect.timeoutOrElse({
          duration: RELEASE_INDEX_TIMEOUT,
          orElse: () =>
            Effect.fail(new CliUpdateError({ reason: "Timed out listing iskra releases." })),
        }),
      );
    const releases = yield* decodeReleaseIndex(body).pipe(
      Effect.mapError(
        () => new CliUpdateError({ reason: "The iskra release index had an unexpected shape." }),
      ),
    );
...
/**
 * The launcher the install scripts leave behind: a symlink at `<bin>/iskra` on
 * POSIX, a `iskra.cmd` shim on Windows. `iskra update` repoints it so the next `iskra`
...
export const repointLauncher = Effect.fn("cli.update.repoint_launcher")(function* (input: {
  ...
  if (platform === "win32") {
    ...
    // itself as argv0; the shim is the `iskra.cmd` next to it only when launched
    // from an install script's bin directory. Find it by searching the
    // directories that would resolve `iskra` on this shell's PATH.
    ...
    yield* fs
      ...
      .pipe(
        Effect.mapError(
          () => new CliUpdateError({ reason: `Could not rewrite the iskra launcher at ${shimPath}.` }),
        ),
      );
  ...
  yield* fs.symlink(input.targetEntryPath, tempLink).pipe(
    ...
    Effect.mapError(
      () =>
        new CliUpdateError({ reason: `Could not repoint the iskra launcher at ${input.launchedAs}.` }),
    ),
  );
 ...
 * spelling in argv0: a launcher symlink or `./iskra` resolves against the
 * working directory, while a bare `iskra` was found on PATH and has to be
 ...
 * only ever sees its own path. Walk PATH for a `iskra.cmd` whose target is the
...
export const findWindowsShim = Effect.fn("cli.update.find_windows_shim")(function* (
  ...
  const environment = yield* HostProcessEnvironment;
  const candidates = [
    ...(environment["ISKRA_INSTALL_BIN_DIR"] ? [environment["ISKRA_INSTALL_BIN_DIR"]] : []),
    ...(environment["PATH"] ?? environment["Path"] ?? "").split(";"),
  ].filter((entry) => entry.trim().length > 0);
  for (const directory of candidates) {
    const shimPath = path.join(directory, "iskra.cmd");
...
const updateFlags = {
  ...
  channel: Flag.choice("channel", CLI_RELEASE_CHANNELS).pipe(
    Flag.withDescription(
      "Release channel to follow. Defaults to the channel this iskra was published on.",
    ),
    Flag.optional,
  ),
...
export const updateCommand = Command.make("update", {
  ...
  Command.withDescription(
    "Download a newer iskra and switch this machine to it, including the background service when one is installed.",
  ),
...
/**
 * A `iskra serve` or `iskra` someone started by hand, as opposed to the one the
...
const belongsToBootService = Effect.fn("cli.update.belongs_to_boot_service")(function* (
  ...
  if (platform === "linux") {
    ...
    return Option.isSome(cgroup) && cgroup.value.includes("/iskra.service");
  }
  if (platform === "darwin") {
...
const runUpdate = Effect.fn("cli.update.run")(function* (input: {
  ...
  if (input.requestedVersion !== undefined && !isExactServiceVersion(input.requestedVersion)) {
    return yield* new CliUpdateError({
      reason: `'${input.requestedVersion}' is not an exact iskra version.`,
    });
  }
  ...
  if (targetChannel === "preview" && currentChannel !== "preview") {
    yield* Console.log(
      [
        `@iskra/cli@${targetVersion} is a preview build.`,
        ...
        `  will have to switch back to ${currentChannel} yourself with \`iskra update --channel ${currentChannel} --allow-downgrade\`.`,
      ].join("\n"),
    );
  ...
  // The unit name is per user, not per Iskra home. Only touch the service when it
  ...
  if (executableCurrent && serviceCurrent) {
    yield* Console.log(
      serviceVersion !== undefined
        ? `iskra and its background service are already on ${targetVersion} (${targetChannel}).`
        : `iskra is already on ${targetVersion} (${targetChannel}).`,
    );
    return;
  }
  if (!input.allowDowngrade && compareExactServiceVersions(targetVersion, newestInstalled) < 0) {
    return yield* new CliUpdateError({
      reason: `@iskra/cli@${targetVersion} is older than the installed ${newestInstalled}. Pass --allow-downgrade to install it anyway.`,
    });
  }
  ...
  yield* Console.log(
    executableCurrent && restartPending
      ...
      : executableCurrent
        ...
        : alreadyOnDisk
          ? `Switching iskra ${currentVersion} -> ${targetVersion} (${targetChannel}, already downloaded).`
          : `Updating iskra ${currentVersion} -> ${targetVersion} (${targetChannel}).`,
  );
  let restartService = false;
  if (serviceInstalled && !serviceCurrent) {
    yield* Console.log(
      "  A background service is installed for this Iskra home. Restarting it interrupts anything running in it: agent turns, terminals, remote clients.",
    );
    if (input.assumeYes) {
      restartService = true;
      ...
      yield* Console.log(
        "  Not a terminal, so the service keeps running its current version. Rerun with --yes to restart it now, or run `iskra service restart` later.",
      );
    }
  }
  ...
  const runtime = yield* ensurePinnedRuntimeInstalled({
    ...
    validate: (paths) =>
      runner
        ...
        .pipe(
          Effect.mapError(
            (cause) =>
              new PinnedRuntimeInstallError({ step: "verifying the downloaded iskra", cause }),
          ),
            ...
            result.code === 0 && /\bv(\S+)\s*$/.exec(result.stdout)?.[1] === targetVersion
              ...
              : Effect.fail(
                  new PinnedRuntimeInstallError({
                    step: "verifying the downloaded iskra",
    ...
    Effect.catchIf(
      (error): error is PinnedRuntimeInstallError =>
        ...
        error.step.startsWith("downloading the iskra release checksums") &&
        String(error.cause).includes("404"),
      () =>
        Effect.fail(
          new CliUpdateError({
            reason: `No release archive was published for iskra@${targetVersion}.`,
          }),
        ),
    ),
  );
  ...
  // is rewritten either way so a later `iskra service restart` lands on the new
  ...
  if (serviceInstalled && !serviceCurrent) {
    yield* BootService.BootService.pipe(
      ...
      Effect.mapError(
        (error) =>
          new CliUpdateError({
            reason: `@iskra/cli@${targetVersion} is installed but the background service could not be ${restartService ? "updated" : "pointed at it"}: ${error.message}`,
          }),
      ),
    );
  ...
  yield* Console.log("");
  yield* Console.log(`iskra ${targetVersion} is installed at ${runtime.entryPath}`);
  if (Option.isSome(repointed)) {
    yield* Console.log(`  ${repointed.value} now runs ${targetVersion}`);
  } else {
    yield* Console.log(`  Run it as ${runtime.entryPath}, or point your \`iskra\` launcher at it.`);
  }
  if (serviceUpdated) {
    ...
    yield* Console.log(
      `  Background service still running ${serviceVersion ?? "an unknown version"}. Run \`iskra service restart\` when you are ready to switch it to ${targetVersion}.`,
    );
  } else if (status.installed && !servesThisHome) {
    yield* Console.log(
      `  The background service serves ${status.installedBaseDir ?? "another Iskra home"} and was left unchanged.`,
    );
  }
