...
/**
 * `iskra theme` - inspect and set the environment's theme. Connected web and
 ...
 * Settings afterwards sticks until the next `iskra theme set`.
 *
...
import { fromJsonStringPretty, fromLenientJson } from "@iskra/shared/schemaJson";
import { BUILT_IN_THEME_IDS, UNPUBLISHABLE_THEME_IDS } from "@iskra/shared/themePalettes";
...
export class ThemeFileInvalidError extends Schema.TaggedError<ThemeFileInvalidError>()(
  ...
  override get message(): string {
    return `${this.filePath} is not a valid theme file. Use a theme exported from Iskra, or a seeded file with name, appearance, canvas, and accent.`;
  }
}
...
export class ThemeTargetMissingError extends Schema.TaggedError<ThemeTargetMissingError>()(
  ...
  override get message(): string {
    return "Provide a theme id or file, or run `iskra theme clear` to remove the theme.";
  }
}

const envIskraHome = Config.string("ISKRA_HOME").pipe(Config.option);

const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  // Same precedence as the rest of the CLI: --base-dir, then ISKRA_HOME,
  // then the default home. A provisioning script exporting ISKRA_HOME must
  // not have this one command silently target the default install.
  const envHome = Option.filter(yield* envIskraHome, (value) => value.trim().length > 0);
...
const themeSetCommand = Command.make("set", {
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // make `iskra theme set ocean` publish ./ocean whenever the cwd happens to
