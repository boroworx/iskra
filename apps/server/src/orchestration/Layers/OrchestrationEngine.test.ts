...
function makeOrchestrationLayer(
  ...
  const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
    prefix: "iskra-orchestration-engine-test-",
  });
...
describe("OrchestrationEngine", () => {
  it.each(["running", "stopped"] as const)(
    ...
    async (status) => {
      const directory = await NodeFSP.mkdtemp(
        NodePath.join(NodeOS.tmpdir(), "iskra-async-questions-"),
      );
  ...
  it("keeps agents across a restart, matching a replay of the event log", async () => {
    const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "iskra-agents-"));
    const databasePath = NodePath.join(directory, "state.sqlite");
  ...
  it("does not regress a generated branch to a stale temporary worktree branch", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
        runtimeMode: "approval-required",
        branch: "iskra/generated-branch-name",
    ...
    await system.run(
      engine.dispatch({
        ...
        expectedBranch: "iskra/1234abcd",
      }),
    );

    const snapshot = await system.readModel();
    expect(snapshot.threads[0]?.branch).toBe("iskra/generated-branch-name");
    await system.dispose();
  });
  ...
  it("allows authoritative worktree bootstrap to assign a temporary branch", async () => {
    ...
    const snapshot = await system.readModel();
    expect(snapshot.threads[0]?.branch).toBe("iskra/1234abcd");
  ...
  it("records command ack duration using the first committed event type", async () => {
    ...
    expect(
      hasMetricSnapshot(snapshots, "iskra_orchestration_command_ack_duration", {
  ...
  it("records failed command dispatches as metric failures", async () => {
    ...
    const snapshots = await system.run(Metric.snapshot);
    expect(
      hasMetricSnapshot(snapshots, "iskra_orchestration_commands_total", {
        commandType: "thread.create",
        aggregateKind: "thread",
        outcome: "failure",
      }),
  ...
  it("stores completed checkpoint summaries even when no files changed", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        checkpointRef: asCheckpointRef("refs/iskra/checkpoints/thread-turn-diff/turn/1"),
        status: "ready",
        files: [],
