...
export class SqliteStateDatabaseMissingError extends Schema.TaggedError<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Iskra once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedError<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.iskra database. Use an isolated --base-dir.";
  }
}
...
const iskraSqliteStateCommand = Command.make(
  "iskra-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Iskra base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Iskra SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(iskraSqliteStateCommand, { version: "0.0.0" }).pipe(
