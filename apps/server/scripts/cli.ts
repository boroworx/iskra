...
const buildExeCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
        ...
        ChildProcess.make(spawnCommand.command, spawnCommand.args, {
          ...
          env: {
            ...
            ...Option.match(config.target, {
              onNone: () => ({}),
              onSome: (target) => ({ ISKRA_PACK_EXE_TARGET: target }),
            }),
          },
          stdout: config.verbose ? "inherit" : "ignore",
          stderr: "inherit",
      ...
      yield* Effect.log(
        "[cli] Built dist-exe/iskra (expects client/, resource-monitor/, and the runtime-external node_modules beside it; scripts/build-cli-archive.ts assembles that tree)",
      );
    }),
).pipe(
 ...
 * every `@iskra/cli-<platform>.tgz` first, `iskra.tgz` (the launcher) last, so
...
const publishCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const scopeDir = path.join(packagesDir, "@iskra");
      const launcherTarball = path.join(packagesDir, "iskra.tgz");
      const platformTarballs = (yield* fs
        .readDirectory(scopeDir)
        .pipe(Effect.orElseSucceed((): ReadonlyArray<string> => [])))
        .filter((entry) => entry.startsWith("iskra-") && entry.endsWith(".tgz"))
        .sort()
        .map((entry) => path.join(scopeDir, entry));
      if (platformTarballs.length === 0) {
        return yield* new ServerCliBuildAssetMissingError({
          assetPath: path.join(scopeDir, "iskra-<platform>.tgz"),
        });
      }
  ...
  Command.withDescription(
    "Publish the @iskra/cli-<platform> tarballs and then the iskra launcher to npm.",
  ),
);
...
const cli = Command.make("cli").pipe(
  Command.withDescription("Iskra server build & publish CLI."),
