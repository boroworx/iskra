...
const ROUTER_HOST = "app.iskra.sh";
const HOSTED_WEB_CHANNEL_COOKIE = "iskra_web_channel";
const LATEST_ORIGIN = "https://latest.app.iskra.sh";
const NIGHTLY_ORIGIN = "https://nightly.app.iskra.sh";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @iskra/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --ignore-scripts --filter '@iskra/scripts...' --filter '@iskra/web...'",
  routes: [
    {
      src: "/__iskra/channel",
