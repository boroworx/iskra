...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with Iskra-specific exact filename icons", () => {
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "iskra-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name, "iskra-file-icon-pnpm");
    assert.equal(
      resolvePierreIconForEntry("pnpm-workspace.yaml", "file")?.name,
      "iskra-file-icon-pnpm",
    );
  });

  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(
      Object.values(ISKRA_PIERRE_ICONS.byFileName).filter((name) => name.startsWith("iskra-")),
    );
    for (const iconName of customIconNames) {
      assert.include(ISKRA_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
