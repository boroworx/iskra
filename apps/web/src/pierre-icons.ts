...
import { VIDEO_FILE_EXTENSIONS } from "@iskra/shared/video";
...
const PIERRE_ICON_SPRITE_ID = "iskra-pierre-file-icon-sprite";

const ISKRA_FILE_ICON_SPRITE = `
<svg xmlns="http://www.w3.org/2000/svg" width="0" height="0" aria-hidden="true">
  <!-- Lucide Film icon, ISC license. -->
  <symbol id="iskra-file-icon-video" viewBox="0 0 24 24">
  ...
  <symbol id="iskra-file-icon-agents" viewBox="0 0 32 32">
  ...
  <symbol id="iskra-file-icon-pnpm" viewBox="0 0 32 32">
...
export const ISKRA_PIERRE_ICONS = {
  set: "complete",
  colored: true,
  spriteSheet: ISKRA_FILE_ICON_SPRITE,
  byFileName: {
    "package.json": "file-tree-builtin-npm",
    "tsconfig.json": "file-tree-builtin-typescript",
    "agents.md": "iskra-file-icon-agents",
    "pnpm-lock.yaml": "iskra-file-icon-pnpm",
    "pnpm-workspace.yaml": "iskra-file-icon-pnpm",
  },
  byFileExtension: Object.fromEntries(
    VIDEO_FILE_EXTENSIONS.map((extension) => [extension, "iskra-file-icon-video"]),
  ),
} satisfies FileTreeIcons;

const completeIconResolver = createFileTreeIconResolver(ISKRA_PIERRE_ICONS);
...
export function ensurePierreIconSprite(): void {
  ...
  container.innerHTML = `${getBuiltInSpriteSheet("complete")}${ISKRA_FILE_ICON_SPRITE}`;
