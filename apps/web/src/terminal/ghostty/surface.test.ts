...
describe("terminalLinkAtPositionWithRange", () => {
  it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
    const cells = [
      cell("🙂"),
      cell(""),
      ...Array.from("https://iskra.sh", (character) => cell(character)),
    ];
    ...
    expect(terminalLinkAtPositionWithRange([row], 0, 2)?.text).toBe("https://iskra.sh");
  ...
  it("refuses links truncated at the viewport edges instead of mis-resolving", () => {
    ...
    const tailCut = [row("https://iskra.sh", false, true)];
    expect(terminalLinkAtPositionWithRange(tailCut, 0, 8)).toBeNull();
    // A partial bottom row is provably complete and still resolves.
    const complete = [row("https://iskra.sh", false), row("", false)];
    expect(terminalLinkAtPositionWithRange(complete, 0, 8)?.text).toBe("https://iskra.sh");
    ...
    const wideFull: GhosttyRow = {
      cells: [
        { ...cell("🙂"), wide: 1 },
        { ...cell(""), wide: 2 },
        ...Array.from("https://iskra.code", (character) => cell(character)),
      ],
      text: "🙂 https://iskra.code",
      isWrapContinuation: false,
      wrapsToNext: true,
    };
    ...
    expect(terminalLinkAtPositionWithRange([unwrittenTail], 0, 8)?.text).toBe("https://iskra.sh");
  });
});
