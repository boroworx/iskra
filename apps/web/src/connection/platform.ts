...
import { bootstrapRemoteBearerSession } from "@iskra/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@iskra/client-runtime/environment";
import { managedRelayAccountChanges, managedRelaySessionAtom } from "@iskra/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@iskra/client-runtime/rpc";
import {
    ...
    const cloudSession = CloudSession.of({
      ...
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Iskra Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Iskra Connect session is unavailable.",
          });
        }
        return token;
      }),
