...
import { resolveWorkEntryToolPresentation } from "@iskra/client-runtime/work-log/presentation";
...
describe("deriveWorkLogEntries", () => {
  ...
  it("preserves MCP server, tool, arguments, and results for expanded display", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "mcp-tool-done",
        kind: "tool.completed",
        summary: "iskra · preview_status",
        payload: {
          itemType: "mcp_tool_call",
          title: "iskra · preview_status",
          toolSurface: "browser",
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.toolTitle).toBe("iskra · preview_status");
  ...
  it.each([
    ...
    (status, displayName) => {
      const data = {
        toolName: "mcp__iskra_code__preview_click",
  ...
  it("keeps MCP payloads while collapsing lifecycle updates", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        kind: "tool.updated",
        summary: "iskra · preview_snapshot",
        payload: {
          itemType: "mcp_tool_call",
          toolCallId: "call-1",
          toolSurface: "browser",
