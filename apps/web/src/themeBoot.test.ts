...
const THEME_STORAGE_KEY = "iskra:theme";
...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "Rose follows a dark OS",
      storage: { [THEME_STORAGE_KEY]: "rose", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "an explicit global dark mode applies to Rose",
      storage: {
        [THEME_STORAGE_KEY]: "rose",
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "iskra:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("rose");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(ROSE_THEME, "light")!.canvas,
    );
  });
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "rose",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "iskra:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("rose");
    expect(boot.isDark).toBe(true);
  });
  ...
  it("ignores malformed custom theme entries before applying a splash", () => {
    ...
    expect(boot.backgroundColor).toBe("#fbfcfe");
    expect(boot.metaContent).toBe("#fbfcfe");
  });
