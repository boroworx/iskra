...
/**
 * Drives the primary environment's Iskra Connect link. Iskra Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Iskra Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[iskra-connect] Could not update Iskra Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Iskra Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Iskra Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Iskra Connect before enabling this."));
        return false;
      }
