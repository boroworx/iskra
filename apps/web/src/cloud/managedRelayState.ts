...
import type { RelayClientDeviceRecord, RelayClientEnvironmentRecord } from "@iskra/contracts/relay";
...
export function useManagedRelayEnvironments() {
  ...
  useEffect(() => {
    if (snapshot.error) {
      console.error("[iskra-cloud] Relay environment listing failed", {
...
export function useManagedRelayDevices() {
  ...
  useEffect(() => {
    if (snapshot.error) {
      console.error("[iskra-cloud] Relay device listing failed", {
        message: snapshot.error,
        traceId: snapshot.errorTraceId,
      });
    }
