import { findErrorTraceId } from "@iskra/client-runtime/errors";
import {
...
import type { RelayClientEnvironmentRecord } from "@iskra/contracts/relay";
...
/**
 * The user's Iskra Connect environments from relay discovery, each with a
...
export function CloudEnvironmentConnectRows({
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Iskra Connect.`,
      });
      return true;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the Iskra Connect environment.";
    const traceId = findErrorTraceId(cause);
    console.error("[iskra-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",
  ...
  return visibleEnvironments.map(({ environment, availability, error }) => {
    ...
    const statusText = savedConnection
      ...
      : availability === "online"
        ? "Iskra Connect · Not added · Relay online"
        : availability === "offline"
          ? "Iskra Connect · Not added · Relay offline"
          : availability === "checking"
            ? "Iskra Connect · Not added · Checking relay status…"
            : (Option.getOrNull(error)?.message ??
              "Iskra Connect · Not added · Relay status unavailable");
    if (selection) {
      return (
        <label
