...
import { wasBootstrapThreadDeleted } from "@iskra/client-runtime/errors";
import { readPastedComposerContext } from "./composerInlineTokenPaste";
import { isPasteAsTextShortcut } from "@iskra/client-runtime/text-paste";
import { type CodexArtifactTemplate } from "@iskra/client-runtime/codex-artifact-templates";
import { effectiveSnoozed, threadWokeAt } from "@iskra/client-runtime/state/thread-settled";
import {
...
import { assistantCitationsToPlainText } from "@iskra/shared/assistantCitations";
...
import { buildTemporaryWorktreeBranchName } from "@iskra/shared/git";
...
} from "@iskra/client-runtime/state/subagentRuntime";
...
import { resolveProviderSkillsForCwd } from "@iskra/client-runtime/providerSkills";
...
export default function ChatView(props: ChatViewProps) {
  ...
  const [resumeCompactionPermanentlyDismissed, setResumeCompactionPermanentlyDismissed] =
    useLocalStorage(
      `iskra:resume-compaction-dismissed:${environmentId}:${activeProviderInstanceId ?? "claudeAgent"}`,
      false,
      Schema.Boolean,
    );
  ...
  // the sent message can stay near the top. Iskra only owns streaming adjustments
  ...
  const onRespondToUserInput = useCallback(
    async (requestId: ApprovalRequestId, answers: Record<string, unknown>) => {
      ...
      const attachmentsByQuestionId = new Map<
        string,
        import("@iskra/contracts").UserInputAttachments[string]
      >();
      for (const question of activePendingUserInput.questions) {
        ...
        attachmentsByQuestionId.set(
          question.id,
          uploaded as import("@iskra/contracts").UserInputAttachments[string],
        );
      }
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Iskra server to browse pull requests."
      />
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
          ...
          >
            ...
            >
              <div
                ...
                className="w-full ps-[calc(env(safe-area-inset-left)+1.5rem)] pe-[calc(env(safe-area-inset-right)+1.5rem)]"
              >
                <div
                  data-chat-composer-stack="true"
                  className="group/composer-stack pointer-events-auto relative z-10 mx-auto w-full max-w-[720px]"
                >
                  {isDraftHeroState ? (
      ...
      {rightPanelPresent && !shouldUseRightPanelSheet && activeThreadRef ? (
        <RightPanelTabs
          mode="inline"
          widthStorageKey={`iskra:preview-panel-width:${activeThreadKey}`}
          open={rightPanelOpen}
          maximized={rightPanelMaximized}
