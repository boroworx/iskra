...
> [!WARNING]
> Iskra Mobile is currently in development and is not distributed yet. If you want to try it out, you can build it from source.
...
- `development`: Expo dev client, installable side-by-side as `Iskra Dev`
- `preview`: persistent internal preview build, installable side-by-side as `Iskra Preview`
- `production`: store/release build as `Iskra`

Run commands from `apps/mobile`.

Iskra Connect is optional and disabled in a fresh clone. Public configuration belongs in the
...
```bash
ISKRA_IOS_PERSONAL_TEAM=1 \
ISKRA_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.iskra.dev \
vp run ios:dev
```
...
```bash
ISKRA_IOS_PERSONAL_TEAM=1 \
ISKRA_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.iskra \
vp run ios:release
```
...
For preview or production EAS environments, set `ISKRA_CLERK_PUBLISHABLE_KEY`,
`ISKRA_CLERK_JWT_TEMPLATE`, and `ISKRA_RELAY_URL`
