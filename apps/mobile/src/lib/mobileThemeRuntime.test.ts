...
describe("mobileThemeRuntime", () => {
  it("keeps the default palette on Uniwind's built-in appearance themes", () => {
    expect(getMobileUniwindThemeName("iskra", "light")).toBe("light");
    expect(getMobileUniwindThemeName("iskra", "dark")).toBe("dark");
  });

  it("maps custom palettes and appearances to registered themes", () => {
    expect(getMobileUniwindThemeName("rose", "dark")).toBe("rose-dark");
  });
