...
import { runAtomCommand, squashAtomCommandFailure } from "@iskra/client-runtime/state/runtime";
import type {
  ChatFileAttachment,
  ChatImageAttachment,
  EnvironmentId,
...
import { PROVIDER_SEND_TURN_SUPPORTED_IMAGE_MIME_TYPES } from "@iskra/contracts";
...
async function uploadFileBytes(
  ...
  const file =
    fileUri === undefined
      ? new File(Paths.cache, `iskra-upload-${uuidv4()}`)
