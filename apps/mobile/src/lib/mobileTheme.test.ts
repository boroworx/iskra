...
import { BUILT_IN_THEME_IDS, BUILT_IN_THEMES, ROSE_THEME } from "@iskra/shared/themePalettes";
...
describe("mobile themes", () => {
  it("declares every runtime theme variable in the static stylesheet", () => {
    const generatedVariables = createMobileThemeVariables(ROSE_THEME.colors, "light");
  ...
  it("changes either theme without switching the active appearance", () => {
    const themeIds = { light: "rose", dark: "grove" } as const;
  ...
  it("maps semantic palette roles onto every mobile color variable", () => {
    const variables = createMobileThemeVariables(ROSE_THEME.colors, "light");
    expect(Object.keys(variables)).toHaveLength(75);
    expect(variables["--color-sheet-solid"]).toBe(
      themeColorToNativeColor(ROSE_THEME.colors.chrome),
    );
    expect(variables["--color-warning"]).toBe(
      themeColorToNativeColor(ROSE_THEME.colors.warningSurface),
    );
    expect(variables["--color-warning-foreground"]).toBe(
      themeColorToNativeColor(ROSE_THEME.colors.warningForeground),
    );
