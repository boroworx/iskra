...
import { renderAssistantCitationsAsText } from "@iskra/shared/assistantCitations";
...
} from "@iskra/client-runtime/codex-artifact-templates";
import { resolveAssetUrl } from "@iskra/client-runtime/state/assets";
...
} from "@iskra/client-runtime/markdown-images";
import { resolveViewedImageAsset } from "@iskra/client-runtime/work-log/presentation";
import {
  renderCodexFileCitationsAsMarkdown,
  splitCodexArtifactTemplateMarkdown,
} from "@iskra/client-runtime/codex-markdown-directives";
import { CHAT_LIST_ANCHOR_OFFSET, resolveChatListAnchoredEndSpace } from "@iskra/shared/chatList";
import { imageMimeType } from "@iskra/shared/image";
...
import { faviconUrlForOrigin } from "@iskra/shared/favicon";
import {
...
import { markdownFileIconSource } from "@iskra/mobile-markdown-text/file-icons";
import { PierreEntryIcon } from "../../components/PierreEntryIcon";
import { markdownLinkIconSource } from "@iskra/mobile-markdown-text/link-icons";
import {
  normalizeNativeMarkdownUrl,
  resolveMarkdownInlineCodePresentation,
  resolveMarkdownLinkIcon,
  resolveMarkdownLinkPresentation,
} from "@iskra/mobile-markdown-text/links";
import {
      ...
      >
        ...
        >
          ...
          <View className="min-w-0 flex-1 gap-1">
            <Text className="font-iskra-medium text-sm text-foreground" numberOfLines={2}>
              {attachment.name}
            </Text>
...
function MarkdownInlineCode(props: {
  ...
  return (
    <NativeText
      className={presentation ? "font-iskra-bold" : "font-mono"}
  ...
  return (
    <View className="my-2 min-w-0 flex-row items-center gap-3 rounded-2xl border border-border bg-card px-3 py-3">
      ...
      <View className="min-w-0 flex-1">
        <Text className="font-iskra-bold text-sm text-foreground" numberOfLines={1}>
      ...
      {props.onUse ? (
        ...
        >
          <Text className="font-iskra-bold text-xs text-foreground">Use template</Text>
        </Pressable>
      ) : null}
    </View>
  );
    ...
    return (
      ...
      >
        <Text
          ...
          className="font-iskra-medium text-sm tabular-nums text-foreground-muted"
        >
          {entry.label}
        </Text>
        <ThreadDisclosureChevron
      ...
      >
        ...
        <View className="shrink-0 flex-row items-center gap-1.5">
          <SymbolView
            ...
            type="monochrome"
          />
          <Text className="font-iskra-medium text-xs text-foreground-muted">{label}</Text>
        </View>
        <View className="h-px flex-1 bg-adaptive-neutral-200-a80-white-a8" />
      </View>
    );
      ...
      return (
        <View className="mb-5 items-end">
          ...
          <View className="mt-1 flex-row items-center justify-end gap-1 pr-0.5">
            <Text className="font-iskra-medium text-xs tabular-nums text-adaptive-neutral-600-400">
...
function UserMessageContent(props: UserMessageContentProps) {
  ...
  const text = replaceComposerContextReferences(props.text, (ref) => {
    ...
    return `[${ref.label}${available ? "" : " (unavailable)"}](iskra-context://v1/${ref.kind}/${ref.contextId})`;
  });
  ...
  return (
    ...
    >
      <View className="max-w-[320px] items-center gap-2">
        <Text className="text-center font-iskra-bold text-lg text-foreground">{props.title}</Text>
