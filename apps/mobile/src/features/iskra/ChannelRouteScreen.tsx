...
import { ActionButton, AgentAvatar, Body, EventLine, Muted, TONE_COLOR } from "./components";
...
type ChannelParams = StaticScreenProps<{ readonly environmentId: string; readonly channelId: string }>;
type AgentDmParams = StaticScreenProps<{ readonly environmentId: string; readonly agentId: string }>;
...
export function AgentDmRouteScreen(props: AgentDmParams) {
  ...
  const dm = channels.find((channel) => channel.kind === "dm" && channel.memberAgentIds.includes(agentId));
  return (
    <Conversation
      environmentId={environmentId}
...
function Conversation(props: {
  ...
  const rows = useMemo(
    () => [...messageRows(messages.data ?? EMPTY_MESSAGES, agents)].reverse(),
  ...
  const send = async () => {
    ...
    const sent =
      agentId !== null
        ? await dmPost({ environmentId, input: { agentId, messageId, body: trimmed } }, "The message was not sent")
        : channelId !== null
          ? await post({ environmentId, input: { channelId, messageId, body: trimmed } }, "The message was not sent")
          : false;
    setSending(false);
    if (sent) setBody("");
  };
...
interface MessageRowModel {
  ...
  readonly notes: ReadonlyArray<{ readonly agentId: string; readonly text: string; readonly undelivered: boolean }>;
}

/** Named authors, one header per author's run of messages, and what each human message's deliveries say. */
          ...
          : (message.deliveries ?? []).flatMap((delivery): MessageRowModel["notes"] => {
              ...
              return delivery.status === "delivered"
                ...
                : delivery.status === "undelivered"
                  ? [{ agentId: delivery.agentId, text: `${name} never read this`, undelivered: true }]
        ...
        <EventLine spark={note?.question === true ? "needsYou" : "idle"}>
          ...
          {note !== null ? (
            <Pressable
              ...
              onPress={() =>
                navigation.navigate("IskraCard", { environmentId: props.environmentId, cardId: note.cardId })
              }
            >
