import type { ProjectEntry } from "@iskra/contracts";
    ...
    >
      ...
      <Text
        className={cn(
          ...
          props.selected
            ? "font-iskra-bold text-foreground"
            : node.ignored
              ? "font-iskra-medium text-foreground-tertiary"
              : "font-iskra-medium text-foreground-secondary",
        )}
        numberOfLines={1}
      >
        {node.name}
      ...
      {node.kind === "directory" && props.loaded ? (
        <Text className="text-2xs font-iskra-medium text-foreground-tertiary">
  ...
  if (props.error && props.entries.length === 0) {
    return (
      <View className="flex-1 bg-sheet px-4 py-5">
        <Text className="text-sm font-iskra-bold text-foreground">Files unavailable</Text>
      ...
      ListEmptyComponent={
        <View className="px-4 py-5">
          {props.isPending ? (
            ...
            <>
              <Text className="text-sm font-iskra-bold text-foreground">No files found</Text>
