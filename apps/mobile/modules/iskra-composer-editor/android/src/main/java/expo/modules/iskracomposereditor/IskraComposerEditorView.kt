...
import expo.modules.iskramarkdowntext.IskraContextChip
import org.json.JSONObject
import kotlin.math.max

class IskraComposerEditorView(context: Context, appContext: AppContext) : ExpoView(
  context,
  appContext
) {
  ...
  private fun applyTokenSpans() {
    ...
    tokens.forEach { token ->
      ...
      editable.setSpan(
        ComposerChipSpan(
          IskraContextChip(
            content = IskraContextChip.Content(
              label = token.label,
              symbol = token.symbol,
              detail = token.detail
            ),
            fontSize = editor.textSize * 0.8f,
            colors = IskraContextChip.Colors(
              accent = IskraContextChip.color(token.accent, chipTheme.chipText),
...
private class ComposerChipSpan(
  private val chip: IskraContextChip
) : ReplacementSpan() {
...
internal class SelectionAwareEditText(context: Context) : EditText(context) {
  ...
  private fun copyContext(cut: Boolean): Boolean {
    ...
    IskraComposerClipboard.write(context, text.substring(start, end), clipboardFragment)
    if (cut) text.delete(start, end)
    return true
  }

  private fun pasteContextOrImages(): Boolean {
    val payload = IskraComposerClipboard.read(context)
    if (payload["html"]?.contains("data-iskra-context-fragment=") == true) {
