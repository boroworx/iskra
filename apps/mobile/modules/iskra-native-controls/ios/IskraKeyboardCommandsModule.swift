...
public final class IskraKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("IskraKeyboardCommands")

    View(IskraKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: IskraKeyboardCommandsView, commands: [String]) in
...
public final class IskraKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.iskraFirstResponder == nil else { return }
...
private extension UIView {
  var iskraFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.iskraFirstResponder { return responder }
    }
    return nil
  }
}
