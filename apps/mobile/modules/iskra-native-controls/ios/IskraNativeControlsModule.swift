...
public final class IskraNativeControlsModule: Module {
  private let presentationSources = IskraPresentationSources()
  private var videoPresentation: IskraNativeVideoPresentation?
  private var filePresentation: IskraNativeFilePresentation?

  public func definition() -> ModuleDefinition {
    Name("IskraNativeControls")
    ...
    View(IskraPresentationSourceView.self) {
      ViewName("PresentationSource")
      Prop("identifier") { (view: IskraPresentationSourceView, identifier: String) in
        view.sources = self.presentationSources
        view.identifier = identifier
      }
    }

    View(IskraContextSheetSizeView.self) {
      ViewName("ContextSheetSize")
      Prop("contentHeight") { (view: IskraContextSheetSizeView, height: Double) in
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/IskraShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("IskraNativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/IskraShowcaseReadyScene"
  ...
  private func presentVideo(url: URL, title: String, sourceIdentifier: String, identifier: String, promise: Promise) throws {
    ...
    guard videoPresentation == nil, filePresentation == nil,
      ...
      throw NSError(
        domain: "IskraNativeVideo",
        code: 2,
        userInfo: [NSLocalizedDescriptionKey: "The video preview is no longer available."]
      )
    }
    let presentation = IskraNativeVideoPresentation(identifier: identifier, url: url, title: title) { [weak self] error in
  ...
  private func presentFile(url: URL, title: String, sourceIdentifier: String,
    ...
    let file = IskraNativeFilePresentation(identifier: identifier, sources: presentationSources,
  ...
  private func shareFile(url: URL, title: String, sourceIdentifier: String, promise: Promise) throws {
    guard let presenter = appContext?.utilities?.currentViewController() else {
      throw NSError(
        domain: "IskraNativePresentation",
        code: 2,
