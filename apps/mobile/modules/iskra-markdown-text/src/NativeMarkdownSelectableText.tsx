...
import { decodeComposerContextFragment } from "@iskra/shared/composerContextClipboard";
import {
  findNodeHandle,
  Image,
  Linking,
...
} from "./IskraMarkdownTextSelectionModule";
...
export function NativeMarkdownSelectableText(props: {
  ...
  // IskraMarkdownText only rebuilds its attributed string during native layout. A
        ...
        return (
          <MarkdownTextPrimitive
            ...
            nativeID={
              Platform.OS === "ios"
                ? chip
                  ? `iskra-chip:${JSON.stringify(chip)}`
                  : run.fileIcon
                    ? `iskra-file:${Image.resolveAssetSource(markdownFileIconSource(run.fileIcon)).uri}`
                    : run.skillName
                      ? "iskra-skill:sf:cube"
                      : linkIcon
                        ? `iskra-link:${Image.resolveAssetSource(markdownLinkIconSource(linkIcon)).uri}`
                        : undefined
                : undefined
            }
