#import "IskraMarkdownText.h"
#import "IskraMarkdownTextShadowNode.h"
#import "IskraMarkdownTextComponentDescriptor.h"
#import "IskraMarkdownTextRun.h"
#import "IskraContextChip.h"
#import <React/RCTConversions.h>
#import <objc/runtime.h>

#import <react/renderer/textlayoutmanager/RCTAttributedTextUtils.h>
#import <react/renderer/components/IskraMarkdownTextSpec/EventEmitters.h>
#import <react/renderer/components/IskraMarkdownTextSpec/Props.h>
#import <react/renderer/components/IskraMarkdownTextSpec/RCTComponentViewHelpers.h>
#import "RCTFabricComponentsPlugins.h"

using namespace facebook::react;

@interface IskraContextChipAccessibilityElement : UIAccessibilityElement
@property(nonatomic, weak) IskraMarkdownTextRun *run;
@end

@implementation IskraContextChipAccessibilityElement
...
@interface IskraContextCopyTextView : UITextView
@property(nonatomic, copy) NSDictionary *contextClipboardConfig;
@end

@implementation IskraContextCopyTextView
...
- (void)copy:(id)sender
{
  ...
  if (encoded && copied.count > 0) {
    ...
    item[@"app.iskra.context-fragment"] = encoded;
    item[@"public.html"] = [[NSString stringWithFormat:@"<pre data-iskra-context-fragment=\"%@\">%@</pre>", attribute, escaped] dataUsingEncoding:NSUTF8StringEncoding];
  }
  UIPasteboard.generalPasteboard.items = @[item];
}
@end

static void IskraMarkdownTextApplyParagraphStyles(
...
static void IskraMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    ...
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    attachment.bounds = CGRectMake(
        ...
        attachmentSize,
        attachmentSize);
    NSDictionary *chip = IskraContextChipPayload(imageUri);
    if (chip != nil) {
      CGSize size = CGSizeMake(attachmentRange.chipWidth, attachmentRange.chipHeight);
      attachment.bounds = IskraContextChipBounds(runAttributes[NSFontAttributeName], size);
      NSString *iconUri = [chip[@"iconUri"] isKindOfClass:NSString.class] ? chip[@"iconUri"] : nil;
      attachment.image = IskraContextChipImage(chip, size, iconUri ? images[iconUri] : nil);
    }
...
@protocol IskraMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface IskraMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<IskraMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<IskraMarkdownOutsideTapTarget>)target;

@end

static const void *IskraMarkdownOutsideTapCoordinatorKey =
    &IskraMarkdownOutsideTapCoordinatorKey;

@implementation IskraMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<IskraMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<IskraMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, IskraMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<IskraMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static IskraMarkdownOutsideTapCoordinator *
IskraMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  IskraMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, IskraMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[IskraMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        IskraMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface IskraMarkdownText () <RCTIskraMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface IskraMarkdownText () <IskraMarkdownOutsideTapTarget>
@end

@implementation IskraMarkdownText {
  UIView * _view;
  IskraContextCopyTextView * _textView;
  IskraMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<IskraMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const IskraMarkdownTextProps>();
    _props = defaultProps;
    ...
    _textView = [[IskraContextCopyTextView alloc] init];
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    IskraMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, IskraMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [IskraMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<IskraMarkdownTextProps const>(_props);
  ...
  IskraMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  IskraMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:[IskraMarkdownTextRun class]]) {
      continue;
    }

    IskraMarkdownTextRun *textChild = (IskraMarkdownTextRun *)child;
    ...
    NSURL *link = [NSURL URLWithString:
        [NSString stringWithFormat:@"iskra-markdown-run://%ld", (long)textChild.tag]];
    if (link != nil) {
  ...
  if (textChanged) {
    ...
    for (auto it = _state->getData().attachmentRanges.rbegin();
      ...
      NSDictionary *chip = IskraContextChipPayload([NSString stringWithUTF8String:it->imageUri.c_str()]);
  ...
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:IskraMarkdownTextRun.class]) continue;
    IskraMarkdownTextRun *run = (IskraMarkdownTextRun *)child;
    run.contextChipInteractive = NO;
  }
  for (const auto &attachmentRange : _state->getData().attachmentRanges) {
    NSDictionary *chip = IskraContextChipPayload(
    ...
    IskraMarkdownTextRun *run = [self childForCharacterRange:range];
    ...
    IskraContextChipAccessibilityElement *element =
        [[IskraContextChipAccessibilityElement alloc] initWithAccessibilityContainer:self];
    element.run = run;
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::IskraMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::IskraMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<IskraMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
...
{
  const auto &oldViewProps = *std::static_pointer_cast<IskraMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<IskraMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == IskraMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == IskraMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == IskraMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == IskraMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const IskraMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (nullable IskraMarkdownTextRun *)childForCharacterRange:(NSRange)characterRange
{
...
- (IskraMarkdownTextRun*)getTouchChild:(CGPoint)location
{
...
{
  IskraMarkdownTextRun *child = [self childForCharacterRange:textItem.range];
  if (![child hasContextMenu] && !child.contextChipInteractive) {
    return defaultAction;
  }

  __weak IskraMarkdownTextRun *weakChild = child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::IskraMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::IskraMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> IskraMarkdownTextCls(void)
{
  return IskraMarkdownText.class;
}
