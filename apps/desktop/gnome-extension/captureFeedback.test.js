...
it("activates the caller, flies its frozen capture, and retains the landed image until disconnect", async () => {
  ...
  await feedback.activate(":1.23", "Iskra");
...
it("waits for Iskra to remap and cancels that wait when disabled", async () => {
  const { feedback } = begin();
  const activation = feedback.activate(":1.23", "Iskra");
  ...
  const cancelled = next.activate(":1.23", "Iskra");
  next.dispose();
  await expect(cancelled).rejects.toThrow("No active capture");
  expect(shell.mapped.size).toBe(0);
});

it("does not animate under reduced motion but still activates Iskra", async () => {
...
it("rejects another sender and locked-session activation", async () => {
  shell.windows = [target()];
  const { feedback } = begin();
  await expect(feedback.activate(":1.99", "Iskra")).rejects.toThrow("No active capture");
  shell.locked = true;
  await expect(feedback.activate(":1.23", "Iskra")).rejects.toThrow("No active capture");
  expect(shell.activate).not.toHaveBeenCalled();
  feedback.dispose();
});
