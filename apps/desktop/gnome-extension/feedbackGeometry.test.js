...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "Iskra");
  expect(findCaptureDestination([window(99, "Iskra"), target], 42, "Iskra")).toBe(target);
  expect(findCaptureDestination([window(99, "Iskra")], 42, "Iskra")).toBeUndefined();
...
it("does not guess between ambiguous windows from the same process", () => {
  expect(
    findCaptureDestination([window(42, "Iskra"), window(42, "Iskra")], 42, "Iskra"),
  ).toBeUndefined();
