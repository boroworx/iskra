...
it("does not activate Iskra until requested, then matches PID and title", async () => {
  ...
  windows = [
    { ...window, id: 1, pid: 999, title: "Iskra" },
    { ...window, id: 2, pid: process.pid, title: "Other Iskra" },
    { ...window, id: 3, pid: process.pid, title: "Iskra" },
  ];
  await snapshot.feedback!.activate("Iskra");
  expect(calls).toContainEqual({ Action: { FocusWindow: { id: 3 } } });
});

it("waits for the restored Iskra window to map instead of polling", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  ...
  handler = async (request, socket) => {
    ...
    if (request === "EventStream")
      send(socket, {
        WindowOpenedOrChanged: { window: { ...window, id: 4, pid: process.pid, title: "Iskra" } },
      });
  };
  await snapshot.feedback!.activate("Iskra");
...
it("rejects ambiguous activation targets", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  windows = [1, 2].map((id) => ({ ...window, id, pid: process.pid, title: "Iskra" }));
  await expect(snapshot.feedback!.activate("Iskra")).rejects.toThrow("More than one");
});
...
it("cancels pending activation when capture feedback is closed", async () => {
  ...
  const activation = expect(snapshot.feedback!.activate("Iskra")).rejects.toThrow("cancelled");
