...
export const HYPRLAND_CAPTURE_EXECUTABLE = "iskra-hyprland-snap-shot";
...
export function hyprlandCaptureExecutable(paths: HyprlandCapturePaths) {
  return NodePath.join(paths.dataHome, "iskra", "hyprland-capture", HYPRLAND_CAPTURE_EXECUTABLE);
}
...
export class HyprlandCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        throw new Error(
          "The Hyprland capture helper is missing from this build. Update or reinstall Iskra.",
        );
...
export async function captureHyprlandWindow(
  ...
  const directory = await NodeFSP.mkdtemp(
    NodePath.join(NodeOS.tmpdir(), "iskra-hyprland-capture-"),
  );
