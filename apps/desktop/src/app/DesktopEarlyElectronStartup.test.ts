...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { ISKRA_HOME: "/home/user/.iskra-test" },
      homeDirectory: "/home/user",
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.iskra-test/userdata/desktop-settings.json");
  ...
  it("resolves the early linux Electron switches", () => {
    const options = resolveEarlyLinuxElectronOptions({
      env: {
        ISKRA_HOME: "/home/user/.iskra-test",
    ...
    assert.deepEqual(options, {
      isDevelopment: true,
      linuxWmClass: "iskra-dev",
      linuxDesktopEntryName: "sh.iskra.Iskra.Development.desktop",
      passwordStore: "gnome-libsecret",
    });
  });

  it("keeps implicit development state under ~/.iskra/dev when ISKRA_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.iskra/dev/desktop-settings.json");
  ...
  it("treats whitespace-only ISKRA_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        ISKRA_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
