...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    ...
    assert.include(entry, "Name=Iskra (Nightly)");
    ...
    assert.include(entry, "MimeType=x-scheme-handler/iskra;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "iskra",
      desktopEntryPath: "/home/alice/.local/share/applications/sh.iskra.Iskra.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the iskra:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/sh.iskra.Iskra.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the iskra:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "sh.iskra.Iskra.desktop", "x-scheme-handler/iskra"],
        },
      ]);
    });
  });
  ...
  it.effect("writes the portal identity without claiming the URL scheme in development", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(unpackaged, {
        environment: {
          isPackaged: false,
          linuxDesktopEntryName: "sh.iskra.Iskra.Development.desktop",
        },
      });
      ...
      assert.equal(
        ...
        "/home/alice/.local/share/applications/sh.iskra.Iskra.Development.desktop",
      );
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/sh.iskra.Iskra.desktop",
        }),
      });
