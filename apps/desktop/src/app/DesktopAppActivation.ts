...
import { HostProcessUserId } from "@iskra/shared/hostProcess";
  ...
  const server = NodeNet.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "Iskra could not process the desktop app request."),
        );
      });
    });
...
>()("@iskra/desktop/app/DesktopAppActivation") {}
