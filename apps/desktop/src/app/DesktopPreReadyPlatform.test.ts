    ...
    it.effect(
      ...
      () => {
        ...
        writeFileSyncMock.mockImplementation((path: string, contents: string) => {
          if (path === "/xdg/applications/sh.iskra.Iskra.desktop") desktopEntry = contents;
        });
        ...
        return Effect.scoped(
          Effect.gen(function* () {
            ...
            assert.equal(identity.desktopName, "sh.iskra.Iskra.desktop");
            assert.include(identity.desktopEntry ?? "", 'Exec="/Applications/current.AppImage" %U');
            assert.include(identity.desktopEntry ?? "", "Name=Iskra (Alpha)");
            assert.include(identity.desktopEntry ?? "", "MimeType=x-scheme-handler/iskra;");
          }),
    ...
    () =>
      Effect.gen(function* () {
        class ClerkShaped extends Context.Service<ClerkShaped, { readonly ready: true }>()(
          "@iskra/desktop/app/DesktopPreReadyPlatform.test/ClerkShaped",
        ) {}
