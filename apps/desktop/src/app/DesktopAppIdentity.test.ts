...
const defaultEnvironmentInput = {
  ...
  processArch: "arm64",
  appVersion: "1.2.3",
  appPath: "/Applications/Iskra.app/Contents/Resources/app.asar",
  isPackaged: true,
  resourcesPath: "/Applications/Iskra.app/Contents/Resources",
      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            // Every path exists, including an IskraCode (Alpha) directory.
            exists: () => Effect.succeed(true),
            readFileString: () =>
              Effect.succeed(input.packageJson ?? '{"iskraCommitHash":"abcdef1234567890"}'),
          }),
        ),
...
describe("DesktopAppIdentity", () => {
  it.effect("never adopts an existing Iskra userData directory", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;

        assert.equal(userDataPath, "/Users/alice/Library/Application Support/iskra");
      }),
    ),
  );
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Iskra (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Iskra (Alpha)");
