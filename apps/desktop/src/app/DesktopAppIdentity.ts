...
const AppPackageMetadata = Schema.Struct({
  iskraCommitHash: Schema.optional(Schema.String),
});
...
>()("@iskra/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.iskraCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
