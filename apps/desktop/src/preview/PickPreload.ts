...
const OVERLAY_ATTRIBUTE = "data-iskra-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--iskra-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--iskra-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--iskra-radius": theme.radius,
    "--iskra-background": theme.background,
    "--iskra-foreground": theme.foreground,
    "--iskra-popover": theme.popover,
    "--iskra-popover-foreground": theme.popoverForeground,
    "--iskra-primary": theme.primary,
    "--iskra-primary-foreground": theme.primaryForeground,
    "--iskra-muted": theme.muted,
    "--iskra-muted-foreground": theme.mutedForeground,
    "--iskra-accent": theme.accent,
    "--iskra-accent-foreground": theme.accentForeground,
    "--iskra-border": theme.border,
    "--iskra-input": theme.input,
    "--iskra-ring": theme.ring,
    "--iskra-font-sans": theme.fontSans,
    "--iskra-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-iskra-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--iskra-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-iskra-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
