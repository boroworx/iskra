...
const withImporter = Effect.fnUntraced(function* () {
  ...
  const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "iskra-import-" });
  ...
  const importer = yield* BrowserImport.BrowserImport.pipe(
    Effect.provide(
      BrowserImport.layer.pipe(
        ...
        Layer.provide(Layer.succeed(HostProcessExecutablePath, "/Applications/Iskra.app")),
    ...
    Effect.gen(function* () {
      ...
      const error = yield* importer
        .importCookies({
          ...
          scope: "persist:iskra-preview-test",
          persistent: true,
        })
        .pipe(Effect.flip);
