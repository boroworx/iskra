...
import { DESKTOP_PREVIEW_RECORDING_CAPTURE_TRIGGER } from "@iskra/contracts";
import type {
...
import { normalizePreviewUrl } from "@iskra/shared/preview";
import {
  BrowserWindow,
  ClipboardItem,
  type Session,
...
const makeNativeOperations = Effect.fn("PreviewManager.makeOperations")(function* (
  ...
  const ensurePlaywrightInjected = Effect.fn("PreviewManager.ensurePlaywrightInjected")(function* (
    ...
    const installed = yield* evaluateWithDebugger<boolean>(
      tabId,
      send,
      "Boolean(globalThis.__iskraPlaywrightInjected)",
      true,
    );
    if (installed) return;
  ...
  const resolveClickPoint = Effect.fn("PreviewManager.resolveClickPoint")(function* (
    ...
    >(
      ...
      `(() => {
          try {
            const injected = globalThis.__iskraPlaywrightInjected;
  ...
  const typeIntoAutomationTarget = Effect.fn("PreviewManager.typeIntoAutomationTarget")(function* (
    ...
    >(
      ...
      `(() => {
          try {
            const element = ${locatorJson ? `(() => { const injected = globalThis.__iskraPlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, true); })()` : "document.activeElement"};
            if (!element) return { notFound: true };
            const textControl =
  ...
  const withNativeKeyReceipt = Effect.fn("PreviewManager.withNativeKeyReceipt")(function* (
    ...
    const { frames, receiptKey } = yield* Effect.acquireRelease(
      attempt(context, () => ({
        ...
        receiptKey: JSON.stringify(`__iskraNativeKey_${NodeCrypto.randomUUID()}`),
      })),
      ({ frames, receiptKey }) =>
        Effect.all(
          frames.map((frame) =>
    ...
    while (true) {
      ...
      if (
        ...
        const world = (yield* send(
          ...
          {
            frameId,
            worldName: "iskra-preview-key-target",
          },
          sessionId,
    ...
    yield* Effect.gen(function* () {
      ...
      if (keySequence.commands?.length) {
        ...
        const selectionKey = yield* encodeJson(
          context,
          `__iskraEditingSelection_${NodeCrypto.randomUUID()}`,
        );
  ...
  const performAutomationScroll = Effect.fn("PreviewManager.performAutomationScroll")(function* (
    ...
    >(
      ...
      `(() => {
        try {
          const target = ${locatorJson ? `(() => { const injected = globalThis.__iskraPlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, true); })()` : "window"};
  ...
  const performAutomationWaitFor = Effect.fn("PreviewManager.performAutomationWaitFor")(function* (
    ...
    while ((yield* currentMillis) <= deadline) {
      ...
      >(
        ...
        `(() => {
              try {
                const selectorMatched = ${locatorJson ? `(() => { const injected = globalThis.__iskraPlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, false) !== null; })()` : "true"};
                const textMatched = ${
...
>()("@iskra/desktop/preview/Manager/PreviewManager") {}
