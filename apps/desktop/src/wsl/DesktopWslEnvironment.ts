...
import { buildRemoteNodeEnvScript } from "@iskra/ssh/tunnel";
...
export class DesktopWslEnvironment extends Context.Service<
  DesktopWslEnvironment,
  {
    ...
    // Proves a staged self-contained runtime can run (`<root>/iskra --version`)
...
>()("@iskra/desktop/wsl/DesktopWslEnvironment") {}
...
// Holds the sha256 of the runtime's `iskra` executable, written when the install
...
// that install wrote.
const WSL_RUNTIME_READY_MARKER = ".iskra-wsl-runtime-ready";
const WSL_RUNTIME_SELECTED_MARKER = ".iskra-wsl-runtime-selected";
...
export const buildWslRuntimeInstallScript = (
  ...
  return [
    ...
    // The runtime is a self-contained `iskra` executable with Node inside, so the
    // readiness proof is the same one the SSH runner and the CLI installers
    // use: the file is executable and `iskra --version` exits 0. That covers the
    // truncated-binary and wrong-arch cases without a separate native probe.
    "runtime_entry_runs() {",
    '  [ -x "$1/iskra" ] && "$1/iskra" --version >/dev/null 2>&1',
    "}",
    // Hashing the entry is what tells a working cache from one whose `iskra` was
    ...
    `  sha256sum "$1/iskra" 2>/dev/null | cut -d ' ' -f 1`,
    "}",
    "runtime_is_ready() {",
    '  [ -f "$ready_marker" ] &&',
    ...
    // The release archive has one top-level `iskra-<version>-linux-<arch>/`
    // directory; strip it so the executable lands at `$runtime_root/iskra`.
    ...
    "  printf 'WSL runtime archive does not contain a working iskra executable\\n' >&2",
    "  exit 1",
    "fi",
...
export const buildWslRuntimeInvalidateScript = (runtimeId: string): string => {
  ...
  return [
    "set -eu",
    `rm -f "$HOME/.iskra/wsl-runtime/${safeRuntimeId}/${WSL_RUNTIME_READY_MARKER}"`,
  ].join("\n");
};
...
const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_BINARY_MISSING_EXIT_CODE
    ? "WSL support is missing from this Iskra build: the packaged Linux node-pty binary was not included. Install a build that includes WSL support."
    : null;
...
const RUNTIME_PROBE_SCRIPT = (linuxAppRoot: string) =>
  [
    ...
    `${shellQuote(`${linuxAppRoot}/iskra`)} --version >/dev/null 2>&1`,
  ].join("\n");
...
const probeWslRuntimeImpl = (
  ...
  Effect.gen(function* () {
    ...
    if (probe.exitCode !== 0) {
      ...
      return {
        ok: false,
        reason: `${linuxAppRoot}/iskra --version failed (exit ${probe.exitCode})${trimmedTail ? `: ${trimmedTail}` : ""}`,
      } as const;
    }
