...
const SERVER_ENTRY_SOURCE = 'console.log("iskra wsl runtime test server");';
...
describe("WSL runtime cache", () => {
  ...
  it("installs through a temporary directory and only reuses valid completed caches", () => {
    ...
    expect(script).toContain(
      "tar -xzf '/mnt/c/Program Files/Iskra/wsl-runtime.tar.gz' -C \"$runtime_tmp\"",
    );
  ...
  it("treats a runtime whose native payload went missing as a cache miss", () => {
    ...
    expect(script).toContain('    [ -f "${candidate%/*}/iskra-wsl-node-pty.json" ] || continue');
  ...
  const createFixture = () => {
    const result = runShell(
      [
        ...
        `printf '%s' '{"arch":"x64"}' > "$stage/node_modules/node-pty/prebuilds/linux-x64/iskra-wsl-node-pty.json"`,
    ...
    return {
      ...
      serverEntry: `${work}/home/.iskra/wsl-runtime/${runtimeId}/apps/server/dist/bin.mjs`,
      installScript,
  ...
  it("prunes old and markerless caches without touching retained, active, locked, or unrelated roots", () => {
    const result = runShell(
      [
        ...
        'make_ready() { mkdir -p "$runtime_parent/$1/apps/server/dist"; printf ready > "$runtime_parent/$1/.iskra-wsl-runtime-ready"; }',
