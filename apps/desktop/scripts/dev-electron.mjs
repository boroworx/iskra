...
const remoteDebuggingPort = process.env.ISKRA_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line iskra/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.ISKRA_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.ISKRA_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--iskra-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--iskra-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
