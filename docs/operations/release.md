...
> For maintainers. Using Iskra? See [docs/user](../user/).
...
- Reads the shared production Iskra Connect relay URL and Clerk client configuration before packaging clients.
...
- Builds a self-contained CLI archive per platform (`iskra-<version>-<platform>-<arch>.tar.gz`, `.zip` on Windows) in the same job as that target's desktop artifact and attaches them to the GitHub Release with a `SHA256SUMS` file, on every channel, for five targets: macOS arm64, Linux x64 and arm64, Windows x64 and arm64. Every archive is built, signed, and smoke-tested on hardware of its own architecture. There is no macOS x64 archive: Node single-executables are unsupported on x64 macOS (the SEA docs list macOS as arm64 only) and the binary segfaults on start; the x64 desktop app is Electron and unaffected.
  - The archive holds the server as a Node single-executable (`scripts/build-cli-archive.ts`), so unpacking it needs neither Node, npm, nor a compiler. It is the only form in which Iskra manages a runtime: the desktop's SSH environments, the boot service, `iskra update`, and the install scripts all download and verify this archive against `SHA256SUMS`. The npm packages exist for people who run `npx @iskra/cli` or `npm install -g @iskra/cli` themselves and carry the same archive contents; nothing in the product installs from npm. The `curl | sh` installers are `scripts/install.sh` and `scripts/install.ps1`; the marketing site copies them into its `public/` at build time (`apps/marketing/scripts/stage-install-scripts.mjs`) and serves them at `iskra.sh/install.sh` and `/install.ps1`.
...
- Publishes the CLI to npm with OIDC trusted publishing from the same workflow file, as the same bytes the GitHub Release carries: `scripts/build-npm-platform-packages.ts` unpacks the five CLI archives into `@iskra/cli-<platform>-<arch>` packages (each with `os`/`cpu` set so npm installs only the matching one) and generates the `iskra` launcher, whose `bin/iskra.js` lists them as `optionalDependencies` and execs the installed executable. `npx @iskra/cli` therefore needs Node only to run the launcher, never to run the server. `node apps/server/scripts/cli.ts publish` publishes the platform packages first and the launcher last, after a `--dry-run` pass over all of them so an auth or scope error fails before anything is live.
  - stable releases publish npm dist-tag `latest`
  - nightly releases publish npm dist-tag `nightly`
  - preview releases publish npm dist-tag `preview`, which nothing resolves unless asked for by name
  - one-time setup: the `@iskra` npm scope (org) must exist, and `iskra` and each `@iskra/cli-<platform>-<arch>` package needs a trusted publisher registered for this workflow file (see below).
...
## Iskra Connect relay deployment
...
```sh
vp run --filter iskra-relay deploy -- --stage "$USER" --env-file .env.local
```
...
The job looks up the `iskra-marketing` project using the existing `VERCEL_TOKEN`
...
- `ISKRA_WEB_ROUTER_URL`: defaults to `https://app.iskra.sh`.
- `ISKRA_WEB_LATEST_DOMAIN`: defaults to `latest.app.iskra.sh`.
- `ISKRA_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.iskra.sh`.

Required Vercel domains:

- `app.iskra.sh`: the router domain users open, updated by stable releases.
- `latest.app.iskra.sh`: channel alias updated by stable releases.
- `nightly.app.iskra.sh`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__iskra/channel?channel=latest` or
`/__iskra/channel?channel=nightly`; the router stores the
`iskra_web_channel` cookie and rewrites future requests on `app.iskra.sh` to
...
through `/__iskra/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.iskra.sh` points at a deployment containing the router
...
- Publishes the CLI npm packages (`iskra` and `@iskra/cli-<platform>-<arch>`) to the `nightly` npm dist-tag using the same nightly version.
- Does not commit version bumps back to `main`.

## Server self-update release invariant

Connected servers update to the client's exact version, not to an npm dist-tag. Every released
desktop or hosted client version must therefore have a matching `@iskra/cli@<version>` package available on
...
For a release smoke test, confirm `npm view @iskra/cli@<version> version` returns the expected version, then
...
verify that the update stops before restart and run `npx @iskra/cli@<version> service update` once on the
...
- Repository slug source:
  - `ISKRA_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
...
(`iskra-<version>-linux-<arch>.tar.gz`, the same arch as the Windows host) built
...
into `~/.iskra/wsl-runtime/sha256-<archive-digest>` inside the selected distro,
...
- The emitted WSL archive is not a Linux CLI release archive: it must unpack to
  a single `iskra-<version>-linux-<arch>` directory holding `iskra`, `client/`, and
...
each `@iskra/cli-<platform>-<arch>.tgz` and finally on `iskra.tgz`, the launcher. The script publishes
...
packages are published per release: `iskra`, `@iskra/cli-darwin-arm64`, `@iskra/cli-darwin-x64`,
`@iskra/cli-linux-arm64`, `@iskra/cli-linux-x64`, `@iskra/cli-win32-arm64`,
`@iskra/cli-win32-x64`.

Checklist:

1. Confirm the npm org owns package `iskra` and the `@iskra` scope exists on npm (create the org if
   it does not).
2. For `iskra` and each `@iskra/cli-<platform>-<arch>` package, configure a Trusted Publisher in the
...
`v0.0.0-test.1`, classifies the run as the stable channel. It publishes `@iskra/cli` with npm dist-tag
`latest`, creates a real GitHub Release, aliases the hosted app to `latest.app.iskra.sh` and
`app.iskra.sh`, and can commit a version bump to `main` in the finalize job. Do not push a test tag
...
2. Create an explicit App ID for `sh.iskra.app` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [Iskra Connect setup](./connect-setup.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.sh.iskra.app` and includes
    Associated Domains.
