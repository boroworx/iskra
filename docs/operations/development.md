...
Flags go directly after the task name, for example `vp run dev --home-dir /tmp/iskra-dev`.
...
Never run a development server against the live `~/.iskra/userdata`.
...
but occupied ports can shift them. `ISKRA_PORT_OFFSET` or `ISKRA_DEV_INSTANCE` can select a
...
`ISKRA_BUNDLED_DEV=0` opts out when debugging bundler differences. Two reload traps matter
...
including services unrelated to Iskra. If you run untrusted services on that hostname, keep
...
```dotenv
ISKRA_DEV_AUTH_TOKEN=<the value generated above>
```

The `iskra.json` Setup Worktree commands on Unix and Windows link that file to each worktree's
...
```sh
export ISKRA_DEV_AUTH_TOKEN="<the value generated above>"
```
