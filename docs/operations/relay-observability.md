...
- `iskra-relay-traces-prod`, the OpenTelemetry trace dataset shared by the Worker, mobile app, and
  first-party relay clients
- `iskra-relay-otel-ingest-prod`, the dataset-scoped Worker ingest token
- `iskra-mobile-otel-ingest-prod`, the dataset-scoped mobile ingest token
- `iskra-relay-client-otel-ingest-prod`, the dataset-scoped first-party relay-client ingest token
- `iskra-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`iskra-relay-traces-dev-julius` for a personal stage.
