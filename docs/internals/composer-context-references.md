...
```text
[label](iskra-context://v1/<kind>/<contextId>)
![label](iskra-context://v1/image/<contextId>)
```

The parser accepts exactly the `iskra-context:` scheme, the `v1` host, one kind segment matching
...
2. A trailing `<iskra_context version="1">` envelope holds one `<context kind id>` entry per unique
...
3. Captured text is data: any `<` that would open or close `iskra_context` or `context` is escaped,
...
is, older messages are upgraded in memory. `ChatMarkdown` renders `iskra-context://` links through
...
chips, a structured fragment under `web application/x-iskra-context-fragment+json`
