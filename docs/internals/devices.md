...
Device panel work over Tailscale and Iskra Connect, including when an SSH host runs the devices.
...
npm-installed at a pinned version into the Iskra home after its matching Device
...
session arbitrary command execution on the host, so Iskra does not. The
...
snapshot model agents need and stays current with its own releases. Iskra prepends
