...
Connect a phone, browser, or another desktop app to Iskra running on a different
machine. That machine must stay running and reachable while you work.

## Iskra Connect

Iskra Connect makes an environment available to your other devices without setting
up router forwarding. In the desktop app on the host, open **Settings →
Connections**, sign in, and enable **Iskra Connect** for that environment.

For a command-line host, run:

```bash
npx @iskra/cli@latest connect
```

Follow the sign-in instructions. Setup offers a
[background service](./background-service.md); if you decline it, start the
server with `npx @iskra/cli serve`. Saving your sign-in alone does not make the machine
reachable.

On your other device, sign in to the same Iskra Connect account and choose the
...
Iskra Connect renews access credentials when needed without disconnecting a healthy
...
```bash
npx @iskra/cli serve --host <private-ip>
```
...
```bash
npx @iskra/cli serve --tailscale-serve
```

For an already-running server:

```bash
npx @iskra/cli pair --tailscale
```
...
`--tailscale-serve-port`. See `npx @iskra/cli pair --help` for other pairing options.
...
**SSH**, and enter a host or SSH alias such as `user@example.com`. Iskra starts
...
the connection stops a server that Iskra launched; a server that was already
...
management is available through `npx @iskra/cli auth --help`.

A session with an open connection stays listed after its access credential
expires.

To remove an environment from Iskra Connect, open your account menu's **Iskra Connect**
page, or **Settings → Iskra Connect** on mobile, and choose **Deregister**. This
revokes its cloud access and frees its host space even when the environment is
offline or has been wiped.

On a command-line host, `iskra connect unlink` disables exposure while retaining
your login; `iskra connect logout` also clears that login. Background-service
...
## Iskra Connect troubleshooting

Run `iskra connect status` on the host to inspect saved authorization and link
configuration. It is not a live reachability check. If the environment appears
offline, run `iskra service status` and read the displayed log. If it disappears
when SSH closes, see [background-service troubleshooting](./background-service.md#troubleshooting).

| Error                                                     | Recovery                                                                                                                                    |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment_link_limit_exceeded` or managed tunnel limit | Deregister an unused environment, then restart Iskra on the host.                                                                         |
| `auth_invalid` or `invalid_bearer`                        | Run `iskra connect login`. If credentials were revoked, run `iskra connect logout`, then `iskra connect` again. Restart the server after signing in. |
| Expired or invalid link proof                             | Check the host's date and time, update Iskra, then restart it.                                                                            |
...
devices. For server version warnings, follow [Updating Iskra](./updating.md).
...
environment**. Iskra restarts without a local server: no local agents or terminals run, WSL
backends stay off, and other devices can no longer connect to this computer. Your projects,
history, and saved connections are kept, and you keep working through pairing, Iskra Connect, or SSH.
