...
Iskra shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened Iskra directly from a
...
- **Iskra Connect** connects computers that are signed in to your account. Run
  `npx @iskra/cli connect` on each computer you want to add, then start Iskra or run
  `npx @iskra/cli serve` so the computer stays available.
- **Add a computer** connects directly to a server on your network or tailnet.
  Start the server with `npx @iskra/cli serve`, then run `npx @iskra/cli pair --tailscale` and
  paste the pairing link. You can also run `npx @iskra/cli serve --host <address>` and
  use `npx @iskra/cli pair` when the server is already reachable on your network.

Saved computers and computers discovered through Iskra Connect are selected by
...
If Iskra cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If Iskra cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

Iskra checks each selected computer for Claude Code and Codex. If an agent is
...
Iskra finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. Iskra keeps the
...
30 days. You can continue those conversations in Iskra.

Conversation import is best effort. Iskra keeps the first user prompt and the
...
conversation so Iskra does not remove user text. It reads one conversation at
