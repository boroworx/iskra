...
Iskra uses Claude Code's login and configuration. Start with the default provider
...
Iskra uses the Claude configuration on the connected server.
...
Select the model you want in Iskra. For an OpenRouter model outside the built-in
...
do not replace the explicit model ID selected in Iskra. Custom models may have
