...
`agent-device` command line, which Iskra sets up for them.
...
Studio's Device Manager. Iskra detects standard SDK locations; set
...
timeline. Agents drive the device through the `agent-device` command line. Iskra
...
local network, Tailscale, and Iskra Connect. Live video needs a secure page
...
Iskra checks common Homebrew and Android SDK locations; custom installations need
...
sessions and stops its Iskra helpers when reachable; simulators keep running.

Iskra provides discovery, streaming, and control. Arrange app builds,
