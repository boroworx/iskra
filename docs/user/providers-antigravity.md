...
Iskra runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for Iskra to confirm
...
hostname. Only that Iskra sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. Iskra's separate Plan mode
...
Iskra keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. Iskra also reads `.gemini/skills` and the
