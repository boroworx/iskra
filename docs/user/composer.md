...
also send files to Iskra through another app's system share sheet.
...
messages survive app restarts. Signing out of Iskra Connect keeps that work on your
...
Iskra remembers your provider, model, and model options for new threads. A
...
Transcription runs on your device. Iskra deletes the temporary audio after
...
Provider commands must start the message to run. Iskra commands such as
...
complete Iskra tells you and leaves the chip for you to remove or replace. A chip whose
...
automatically. HTML previews cannot access your Iskra session.
