...
Shift keys together. Iskra attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Iskra is in front captures Iskra itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Iskra refuses shortcuts
...
terminal contents. If an app is slow to answer, Iskra attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **Iskra SnapShots** extension during setup. It is installed
...
shortcut does not fire, check Iskra under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. Iskra shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. Iskra shows the exact binding it will
...
**Other Wayland desktops.** Iskra uses the desktop's screenshot portal when it can capture the
