...
Iskra must start again on that machine;
...
```sh
npx @iskra/cli@<client-version> service update
```
...
For a foreground server, the copied command is `npx @iskra/cli@<client-version>`. Add
