...
For workspace mode, a project's `iskra.json` preference applies when the project has no override.
...
Iskra only pulls when it can fast-forward and the checkout has no changed files, untracked files,
