#!/bin/sh
# Installs the Iskra CLI from a GitHub Release archive. Needs only sh, tar,
...
#   ISKRA_INSTALL_BIN_DIR   where the `iskra` symlink goes (default: ~/.local/bin)
...
repo="boroworx/iskra"
base_url="${ISKRA_RELEASE_BASE_URL:-https://github.com/${repo}/releases/download}"
t3_home="${ISKRA_HOME:-$HOME/.iskra}"
bin_dir="${ISKRA_INSTALL_BIN_DIR:-$HOME/.local/bin}"

fail() {
  printf 'iskra install: %s\n' "$1" >&2
  exit 1
}
...
channel="${ISKRA_CHANNEL:-stable}"
version="${ISKRA_VERSION:-}"
if [ -z "$version" ]; then
  ...
  case "$channel" in
    ...
    *) fail "ISKRA_CHANNEL must be stable, nightly, or preview" ;;
  esac
  tmp_index="$(mktemp)"
  ...
  rm -f "$tmp_index"
  [ -n "$version" ] || fail "could not find a ${channel} release; set ISKRA_VERSION"
fi
case "$version" in
  *-preview.*)
    printf '%s\n' \
      "iskra ${version} is a preview build." \
...
if [ -f "${target_dir}/.install-complete" ] && [ "$(cat "${target_dir}/.install-complete")" = "$version" ]; then
  ...
  if [ "$fetch_status" -eq 44 ]; then
    fail "iskra ${version} has no release archive for ${platform}-${arch}; releases before the self-contained CLI can only be installed with \`npm install -g @iskra/cli@${version}\`"
  ...
  "${staging}/iskra" --version >/dev/null || fail "the downloaded executable does not run"
...
mkdir -p "$bin_dir"
ln -sfn "${target_dir}/iskra" "${bin_dir}/iskra"
printf 'Installed iskra %s\n  %s -> %s\n' "$version" "${bin_dir}/iskra" "${target_dir}/iskra"
case ":${PATH}:" in
  *":${bin_dir}:"*) ;;
  *) printf 'Add %s to your PATH to run `iskra`.\n' "$bin_dir" ;;
esac
