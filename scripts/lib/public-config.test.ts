...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.ISKRA_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.ISKRA_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.ISKRA_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.ISKRA_RELAY_URL).toBeUndefined();
    expect(env.VITE_ISKRA_RELAY_URL).toBeUndefined();
    expect(env.ISKRA_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.ISKRA_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.ISKRA_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.ISKRA_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.ISKRA_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.ISKRA_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "ISKRA_CLERK_PUBLISHABLE_KEY=pk_root\nISKRA_CLERK_JWT_TEMPLATE=template_root\nISKRA_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nISKRA_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "ISKRA_CLERK_PUBLISHABLE_KEY=pk_local\nISKRA_CLERK_JWT_TEMPLATE=template_local\nISKRA_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nISKRA_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).ISKRA_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          ISKRA_CLERK_PUBLISHABLE_KEY: "pk_ci",
          ISKRA_CLERK_JWT_TEMPLATE: "template_ci",
          ISKRA_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          ISKRA_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      ISKRA_RELAY_URL: "https://ci.example.test",
      VITE_ISKRA_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        ISKRA_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_ISKRA_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          ISKRA_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          ISKRA_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          ISKRA_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          ISKRA_RELAY_URL: "https://relay.example.test",
          ISKRA_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          ISKRA_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          ISKRA_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      ISKRA_RELAY_URL: "https://relay.example.test",
      VITE_ISKRA_RELAY_URL: "https://relay.example.test",
      ISKRA_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      ISKRA_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      ISKRA_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "iskra-public-config-"));
