 ...
 * archive: the `iskra` binary, the web client, the resource monitor, and a
 ...
 * archive stem):
 *
 *   iskra | iskra.exe          the single-executable
...
export function cliArchiveStem(version: string, platform: BuildPlatform, arch: BuildArch): string {
  return `iskra-${version}-${cliArchivePlatformKey(platform, arch)}`;
}
...
const MacSigningConfig = Config.all({
  identity: Config.string("ISKRA_CLI_MAC_SIGN_IDENTITY").pipe(Config.option),
...
const signMacArchiveContents = Effect.fn("signMacArchiveContents")(function* (input: {
  ...
  const libraries = (yield* fs.readDirectory(input.contentDir, { recursive: true }))
    .filter(
      (entry) =>
        ...
        entry.endsWith("iskra-resource-monitor"),
    )
  ...
  if (identity === "-") {
    yield* Effect.log("[cli-archive] Signed ad hoc (no ISKRA_CLI_MAC_SIGN_IDENTITY).");
    return;
  }
  ...
  const notarizeZip = path.join(path.dirname(input.executablePath), ".notarize-iskra.zip");
  yield* runCommand(
  ...
  yield* Effect.log("[cli-archive] Notarized iskra.");
});
...
/** Signs iskra.exe through the same Azure Trusted Signing setup the installer uses. */
const signWindowsExecutable = Effect.fn("signWindowsExecutable")(function* (
  ...
  yield* runCommand(
    ...
    "Invoke-TrustedSigning iskra.exe",
  );
  yield* Effect.log("[cli-archive] Signed iskra.exe (Azure Trusted Signing).");
});
...
const buildCliArchive = Effect.fn("buildCliArchive")(function* (input: {
  ...
  const executableName = input.platform === "win" ? "iskra.exe" : "iskra";
  // tsdown suffixes cross-built executables with their target (iskra-darwin-x64);
  // a host build is plain iskra. Prefer the exact target when both exist.
  const targetKey = `${input.platform === "mac" ? "darwin" : input.platform}-${input.arch}`;
  const targetExecutable = path.join(
    serverDir,
    "dist-exe",
    `iskra-${targetKey}${input.platform === "win" ? ".exe" : ""}`,
  );
  ...
  yield* requireInput(path.join(webClient, "index.html"), "Run `vp run --filter iskra build` first.");
  yield* requireInput(
    resourceMonitorDir,
  ...
  const stageRoot = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-cli-archive-" });
...
).pipe(Command.withDescription("Package the iskra single-executable into a per-platform archive."));
