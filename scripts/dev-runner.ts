...
import { resolveGitWorktreePath, resolveWorktreeIskraHome } from "@iskra/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@iskra/shared/hostProcess";
...
export const DEFAULT_ISKRA_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".iskra"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@iskra/contracts",
    "--filter=@iskra/web",
    "--filter=@iskra/cli",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=@iskra/cli", "dev"],
  "dev:web": ["run", "--filter=@iskra/web", "dev"],
  "dev:desktop": ["run", "--filter=@iskra/desktop", "--filter=@iskra/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`ISKRA_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedError<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("ISKRA_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("ISKRA_PORT_OFFSET"),
  devInstance: optionalStringConfig("ISKRA_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    ...
    return Effect.succeed({
      ...
      source: `ISKRA_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric ISKRA_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed ISKRA_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_ISKRA_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly iskraHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .iskra-dev > ambient ISKRA_HOME) is resolved
    // by the caller; an unset iskraHome here genuinely means "use the default".
    const configuredBaseDir = iskraHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.ISKRA_HOME = resolvedBaseDir;
    } else {
      delete output.ISKRA_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed iskra service (an
    // agent working inside Iskra), these leak through and the child server
    // fails startup with "The service launcher started a different iskra version"
    // (serviceLauncherClient.ts resolveStartup).
    delete output.ISKRA_SERVICE_LAUNCHER_CONTEXT;
    delete output.ISKRA_BOOT_SERVICE_UNIT;

    if (!isDesktopMode) {
      output.ISKRA_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.ISKRA_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.ISKRA_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.ISKRA_PORT = String(serverPort);
    ...
    if (!isDesktopMode && host !== undefined) {
      output.ISKRA_HOST = host;
    }

    if (!isDesktopMode) {
      output.ISKRA_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.ISKRA_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.ISKRA_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.ISKRA_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.ISKRA_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.ISKRA_MODE = "web";
      delete output.ISKRA_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`ISKRA_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["ISKRA_PORT_OFFSET", "ISKRA_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.iskra-dev` — see @iskra/shared/devHome for why this must
    // outrank an ambient ISKRA_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeIskraHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedIskraHome =
      (input.iskraHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.ISKRA_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      iskraHome: resolvedIskraHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.ISKRA_HOME ?? (yield* DEFAULT_ISKRA_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.ISKRA_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          // a few chunk requests. Only defaulted, so ISKRA_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.ISKRA_BUNDLED_DEV === undefined) {
            env.ISKRA_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  iskraHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Iskra data directory; runtime state is stored under userdata (equivalent to ISKRA_HOME). Inside a git worktree this defaults to that worktree's own .iskra-dev so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to ISKRA_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("ISKRA_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to ISKRA_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("ISKRA_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to ISKRA_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("ISKRA_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to ISKRA_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("ISKRA_PORT")),
  ),
