 ...
 * behind `npx @iskra/cli` / `npm i -g iskra`: one `@iskra/cli-<platformKey>` package per
 * archive holding the archive's contents verbatim, plus the `iskra` launcher
 ...
 *
 *   @iskra/cli-<platformKey>/      archive contents flattened + package.json
 *   @iskra/cli-<platformKey>.tgz   the same tree as an npm tarball
 *   iskra/                             launcher: package.json, bin/iskra.js, README.md
 *   iskra.tgz                          the launcher as an npm tarball
 *
 ...
 */
import { legacyCliLauncherScript } from "@iskra/shared/legacyCliLauncher";
...
import { fromJsonStringPretty } from "@iskra/shared/schemaJson";
import { isCommandAvailable } from "@iskra/shared/shell";
import serverPackageJson from "../apps/server/package.json" with { type: "json" };

import { windowsSystemTar } from "./build-cli-archive.ts";

export const NPM_PLATFORM_PACKAGE_SCOPE = "@iskra";
export const NPM_LAUNCHER_PACKAGE_NAME = "iskra";
...
export function npmPlatformPackageManifest(platformKey: CliArchivePlatformKey, version: string) {
  ...
  return {
    ...
    version,
    description: `Iskra CLI executable for ${platformKey}`,
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    os: [os],
    cpu: [cpu],
    files: ["iskra", "iskra.exe", "client", "resource-monitor", "node_modules"],
...
export function npmPlatformPackageReadme(platformKey: CliArchivePlatformKey): string {
  return [
    ...
    "",
    `The Iskra CLI executable for ${platformKey}. Do not install this package directly:`,
    ...
    "```",
    "",
    "Source and documentation: https://github.com/boroworx/iskra",
    "",
  ].join("\n");
}

/** package.json for the `iskra` launcher. No engines: bin/iskra.js is trivial CJS. */
export function npmLauncherPackageManifest(
  ...
  return {
    ...
    version,
    description: "Iskra CLI. Installs the self-contained executable for this platform.",
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    bin: { iskra: "./bin/iskra.js" },
    files: ["bin", "dist"],
...
/**
 * The launcher every `npx @iskra/cli` runs. Plain CommonJS with no dependencies so it
...
try {
  ...
  process.stderr.write(
    [
      "iskra: no Iskra CLI build is available for this platform (" + key + ").",
      "Supported platforms: " + SUPPORTED.join(", ") + ".",
      "If yours is listed, reinstall iskra so npm fetches its optional dependency.",
      "The desktop app and release archives are at https://github.com/boroworx/iskra/releases",
      "",
    ].join("\\n"),
  );
  process.exit(1);
...
const executable = join(packageDir, process.platform === "win32" ? "iskra.exe" : "iskra");
const result = spawnSync(executable, process.argv.slice(2), { stdio: "inherit" });
if (result.error) {
  process.stderr.write("iskra: failed to start " + executable + ": " + result.error.message + "\\n");
  process.exit(1);
}
...
const stagePlatformPackage = Effect.fn("stagePlatformPackage")(function* (input: {
  ...
  const executableName = input.key.startsWith("win32") ? "iskra.exe" : "iskra";
...
/** Writes the launcher package (package.json, bin/iskra.js, README) and its tarball. */
const stageLauncherPackage = Effect.fn("stageLauncherPackage")(function* (input: {
  ...
  const launcherScript = path.join(stageDir, "bin/iskra.js");
...
const command = Command.make(
  "build-npm-platform-packages",
  {
    archivesDir: Flag.string("archives-dir").pipe(
      Flag.withDescription("Directory holding the release's iskra-<version>-<platform> archives."),
    ),
  ...
  Command.withDescription(
    "Build the iskra launcher and @iskra/cli-<platform> npm packages from CLI release archives.",
  ),
);
