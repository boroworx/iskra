...
// The WSL runtime is the Linux CLI release archive (iskra-<version>-linux-<arch>
...
const BuildEnvConfig = Config.all({
  ...
  // Path to the Linux CLI release archive (iskra-<version>-linux-x64.tar.gz) built
  // by the build_linux_cli CI job. The Windows build embeds it verbatim as the
  // WSL runtime.
  wslRuntime: Config.string("ISKRA_DESKTOP_WSL_RUNTIME").pipe(Config.option),
});
...
export const validateWindowsPackagedPayload = Effect.fn(
  ...
  // directory is named iskra-<version>-linux-<arch>.
  readonly appVersion: string;
  ...
  if (hasWslArchive && hasWslArchiveHash) {
    ...
    // desktop app's WSL install script relies on that layout to find `iskra`.
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  wslRuntime: Flag.string("wsl-runtime").pipe(
    Flag.withDescription(
      "Path to the Linux CLI release archive (iskra-<version>-linux-x64.tar.gz) to embed as the WSL runtime of a Windows build (env: ISKRA_DESKTOP_WSL_RUNTIME).",
    ),
    Flag.optional,
  ),
}).pipe(
