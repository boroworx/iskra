...
/** A tar.gz laid out like build-cli-archive.ts writes, with a stub `iskra` that echoes its args. */
const makeFakeArchives = Effect.fn("test.makeFakeArchives")(function* () {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-npm-packages-test-" });
  const archivesDir = path.join(root, "archives");
  yield* fs.makeDirectory(archivesDir);
  for (const key of KEYS) {
    const stem = `iskra-${VERSION}-${key}`;
    ...
    yield* fs.writeFileString(
      path.join(contentDir, "iskra"),
      `#!/bin/sh\necho "stub ${key} $*"\nexit 7\n`,
    );
    yield* fs.chmod(path.join(contentDir, "iskra"), 0o755);
  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        ["@iskra/cli-darwin-arm64", "@iskra/cli-linux-x64", "iskra"],
      );
      for (const output of outputs) {
        assert.isTrue(yield* fs.exists(output.tarball), output.tarball);
      }

      const linuxDir = path.join(fixture.outputDir, "@iskra/cli-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@iskra/cli-linux-x64");
      ...
      assert.deepStrictEqual(outputEntries.sort(), ["@iskra", "iskra", "iskra.tgz"]);
      ...
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@iskra/cli-linux-x64.tgz")],
      ...
      assert.isTrue(
        lines.some((line) => /^-rwxr-xr-x .* package\/iskra$/.test(line)),
        listing.stdout,
      );
      ...
      if (KEYS.some((key) => key === `${hostPlatform}-${hostArch}`)) {
        const passthrough = yield* run(process.execPath, ["bin/iskra.js", "serve", "--port", "1234"], {
          cwd: launcherDir,
          env,
        });
        assert.equal(
        ...
        const unpack = yield* run(
          "tar",
          ["-xf", path.join(fixture.outputDir, "iskra.tgz"), "-C", installedLauncher],
          {
            cwd: fixture.root,
          },
        );
      ...
      const unsupported = yield* run(process.execPath, ["bin/iskra.js", "--version"], {
        cwd: launcherDir,
      ...
      assert.include(unsupported.stderr, "https://github.com/boroworx/iskra/releases");
    }),
  );
});
