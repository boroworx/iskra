...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="Iskra-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit iskra-icon.png
git config user.name 'iskra-ci'
git config user.email 'iskra-ci@users.noreply.github.com'
git add -A
