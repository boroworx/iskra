...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "iskra-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "iskra-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "ISKRA_MOBILE_OTLP_TRACES_DATASET=iskra-mobile-traces-dev",
        "ISKRA_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "ISKRA_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "ISKRA_RELAY_CLIENT_OTLP_TRACES_DATASET=iskra-relay-client-traces-dev",
        "ISKRA_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
  ...
  it("replaces stale values while preserving unrelated entries", () => {
    expect(
      reconcileRootEnvPublicConfig(
        [
          "ISKRA_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "ISKRA_RELAY_URL=https://old.example.test",
          "ISKRA_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "ISKRA_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "ISKRA_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "ISKRA_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "ISKRA_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "ISKRA_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "ISKRA_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "ISKRA_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "ISKRA_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
