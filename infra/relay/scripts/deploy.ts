...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    ISKRA_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    ISKRA_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    ISKRA_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    ISKRA_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    ISKRA_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    ISKRA_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("ISKRA_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("ISKRA_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "IskraRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Iskra relay through Alchemy."));
