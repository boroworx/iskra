...
describe("ssh tunnel scripts", () => {
  it("installs and runs the release archive without Node, npm, or npx", () => {
    const script = buildRemoteIskraRunnerScript(ARCHIVE);

    assert.include(script, "ISKRA_ARCHIVE_VERSION='1.2.3-preview.20260911.4'");
    assert.include(script, "ISKRA_NODE_SCRIPT_PATH=''");
    assert.include(
      script,
      "ISKRA_RELEASE_BASE_URL='https://github.com/boroworx/iskra/releases/download'",
    );
    assert.include(script, 'ISKRA_RUNTIME_DIR="$HOME/.iskra/runtime/versions/$ISKRA_ARCHIVE_VERSION"');
    assert.include(script, 'ISKRA_ARCHIVE="iskra-$ISKRA_ARCHIVE_VERSION-$ISKRA_PLATFORM-$ISKRA_ARCH.tar.gz"');
    assert.include(script, "SHA256SUMS");
    assert.include(script, 'exec "$ISKRA_RUNTIME_DIR/iskra" "$@"');
    assert.notInclude(script, "npx");
    assert.notInclude(script, "npm exec");
    assert.notInclude(script, "@iskra/cli@latest");
    assert.notInclude(script, 'exec iskra "$@"');
    ...
    assert.include(script, 'while ! mkdir "$ISKRA_LOCK" 2>/dev/null; do');
    assert.include(script, 'mv "$ISKRA_LOCK/pid.tmp" "$ISKRA_LOCK/pid"');
    assert.include(script, 'if ! kill -0 "$ISKRA_LOCK_OWNER" 2>/dev/null; then');
    assert.include(script, 'if [ "$ISKRA_LOCK_UNOWNED" -ge 5 ]; then');
    assert.include(script, 'if [ "$ISKRA_LOCK_WAITED" -ge 360 ]; then');
    assert.include(script, '"$ISKRA_STAGING/SHA256SUMS" 30');
    assert.include(script, '"$ISKRA_STAGING/$ISKRA_ARCHIVE" 240');
    assert.notInclude(script, "ISKRA_LOCK_CANDIDATE");
    assert.notInclude(script, "-mmin");
    assert.equal(script.split("if ! t3_runtime_ready; then").length - 1, 2);
    assert.isBelow(
      script.indexOf('"$ISKRA_STAGING/iskra" --version'),
      script.indexOf('> "$ISKRA_STAGING/.install-complete"'),
    );
    ...
    assert.isBelow(
      ...
      script.indexOf('exec node "$ISKRA_NODE_SCRIPT_PATH" "$@"'),
    );
    assert.isBelow(
      script.indexOf('exec node "$ISKRA_NODE_SCRIPT_PATH" "$@"'),
      script.indexOf("ISKRA_ARCHIVE_VERSION="),
    );

    const launch = buildRemoteLaunchScript({
      ...ARCHIVE,
      releaseBaseUrl: "https://mirror.example/iskra/",
    });
    assert.include(launch, "ISKRA_ARCHIVE_MODE=1");
    assert.include(launch, "ISKRA_RELEASE_BASE_URL='https://mirror.example/iskra'");
    ...
    assert.include(buildRemoteLaunchScript(NODE_SCRIPT), "ISKRA_ARCHIVE_MODE=0");
  });
  ...
  it("rejects archive versions that are not a single exact version segment", () => {
    for (const archiveVersion of [
      ...
      assert.throws(
        () => buildRemoteIskraRunnerScript({ archiveVersion }),
        SshInvalidArchiveVersionError,
        undefined,
        archiveVersion,
      );
    ...
    assert.include(
      buildRemoteIskraRunnerScript(ARCHIVE),
      "ISKRA_ARCHIVE_VERSION='1.2.3-preview.20260911.4'",
    );
  });
  ...
  it("refuses to build a runner with neither an archive version nor a node script", () => {
    for (const input of [undefined, {}, { archiveVersion: "  " }, { nodeScriptPath: null }]) {
      assert.throws(() => buildRemoteIskraRunnerScript(input), SshMissingRunnerError);
    }
    assert.throws(() => buildRemoteLaunchScript(), SshMissingRunnerError);
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteIskraRunnerScript(NODE_SCRIPT);

    assert.include(script, "ISKRA_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("builds the remote iskra runner with a node script override", () => {
    const script = buildRemoteIskraRunnerScript({
      ...NODE_SCRIPT,
    ...
    assert.include(
      script,
      "ISKRA_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$ISKRA_NODE_SCRIPT_PATH" "$@"');
    assert.include(script, "ISKRA_ARCHIVE_VERSION=''");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `ISKRA_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for ISKRA_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
    assert.notInclude(script, "npx");
  });

  it("uses the remote iskra runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(devLaunch, `ISKRA_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(launch, "Remote Iskra server did not become ready");
    ...
    assert.include(launch, "ISKRA_ARCHIVE_VERSION='1.2.3-preview.20260911.4'");
    assert.include(
          ...
          if (args.includes("sh")) {
            ...
            if (mode === "failed stop" && stopCommandCount === 1) {
              return {
                ...
                stderr: Stream.make(
                  new TextEncoder().encode("Remote Iskra server did not stop within 2 seconds.\n"),
                ),
              };
            }
        ...
        if (mode === "failed stop") {
          ...
          if (Result.isFailure(disconnected)) {
            ...
            assert.equal(
              ...
              "Remote Iskra server did not stop within 2 seconds.",
            );
          }
        } else {
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "iskra-archive-runner-" });
        const releaseBaseUrl = yield* makeMirror(root);
        const runner = `${root}/run-iskra.sh`;
        yield* fs.writeFileString(
          runner,
          buildRemoteIskraRunnerScript({ archiveVersion, releaseBaseUrl }),
        );
        ...
        for (const result of results) {
          ...
          assert.include(result.stdout, `iskra v${archiveVersion}`);
        }
        const versionsDir = `${home}/.iskra/runtime/versions`;
