import type { DesktopSshEnvironmentBootstrap, DesktopSshEnvironmentTarget } from "@iskra/contracts";
import {
...
export interface SshEnvironmentManagerOptions {
  readonly resolveCliRunner?: Effect.Effect<RemoteIskraRunnerOptions>;
}
...
remote_node_satisfies_engine() {
  ISKRA_NODE_ENGINE_RANGE=@@ISKRA_NODE_ENGINE_RANGE@@
  if [ -z "$ISKRA_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$ISKRA_NODE_ENGINE_RANGE" <<'NODE'
@@ISKRA_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for ISKRA_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$ISKRA_NODE_BIN/node" ]; then
        PATH="$ISKRA_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
@@ISKRA_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.iskra/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.iskra"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-iskra.sh"
RUNNER_NEXT="$STATE_DIR/run-iskra.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@ISKRA_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@ISKRA_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@ISKRA_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@ISKRA_READY_TIMEOUT_MS@@"; then
...
STATE_DIR="$HOME/.iskra/ssh-launch/@@ISKRA_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.iskra"
RUNNER_FILE="$STATE_DIR/run-iskra.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteNodeEnvScript(input?: RemoteIskraRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      ISKRA_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      ISKRA_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}
...
export function buildRemoteLaunchScript(input?: RemoteIskraRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    ...
    ISKRA_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    ISKRA_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    ISKRA_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    ISKRA_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    ISKRA_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    ISKRA_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteIskraRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    ISKRA_STATE_KEY: remoteStateKey(target),
    ISKRA_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteIskraRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteIskraRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteIskraRunnerOptions;
...
>()("@iskra/ssh/tunnel/SshEnvironmentManager") {
