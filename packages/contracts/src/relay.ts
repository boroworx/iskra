...
}).annotate({ description: "Links an authenticated cloud user to an Iskra environment." });
...
>()("@iskra/contracts/relay/RelayClientPrincipal") {}
...
>()("@iskra/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Iskra Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["iskra-mobile", "iskra-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "iskra-mobile" as const;
export const RelayWebClientId = "iskra-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Iskra Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Iskra environments, connecting authorized clients, and publishing agent activity.",
  );
