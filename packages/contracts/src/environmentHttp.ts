...
>()("@iskra/contracts/environmentHttp/EnvironmentAuthenticatedPrincipal") {}
...
class EnvironmentMetadataHttpApi extends HttpApiGroup.make("metadata").add(
  HttpApiEndpoint.get("descriptor", "/.well-known/iskra/environment", {
...
class EnvironmentConnectHttpApi extends HttpApiGroup.make("connect")
  ...
  .add(
    HttpApiEndpoint.post("health", "/api/iskra-connect/health", {
  ...
  .add(
    HttpApiEndpoint.post("iskraMintCredential", "/api/iskra-connect/mint-credential", {
      payload: RelayCloudMintCredentialRequest,
      success: RelayEnvironmentMintResponse,
      error: EnvironmentHttpCloudErrors,
    }),
