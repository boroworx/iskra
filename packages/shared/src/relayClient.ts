...
const CLOUDFLARED_PATH_ENV_NAME = "ISKRA_CLOUDFLARED_PATH";
  ...
  const installUnlocked = Effect.fn("cloudflared.installUnlocked")(function* (
    ...
    if (!releaseAsset) {
      return yield* new RelayClientInstallError({
        ...
        message: `Iskra does not provide a managed relay client binary for ${platform}-${arch}.`,
      });
    }
