...
import { resolveGitWorktreePath, resolveWorktreeIskraHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "iskra-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeIskraHome", () => {
  it.effect("answers with .iskra-dev before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeIskraHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".iskra-dev"));
