...
// oxlint-disable-next-line iskra/no-global-process-runtime -- This test launches a real host executable.
const hostPlatform = NodeOS.platform();
// oxlint-disable-next-line iskra/no-global-process-runtime -- Match the real executable used by the subprocess.
...
it.skipIf(hostPlatform === "win32").each(["npm", "archive"] as const)(
  ...
  async (distribution) => {
    const root = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "iskra-legacy-launcher-"));
    const entry = NodePath.join(root, "node_modules/@iskra/cli/dist/bin.mjs");
    const executable =
      distribution === "archive"
        ? NodePath.join(root, "iskra")
        : NodePath.join(root, `node_modules/@iskra/cli-${hostPlatform}-${hostArch}/iskra`);
