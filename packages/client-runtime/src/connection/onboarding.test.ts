import { AuthStandardClientScopes, EnvironmentId } from "@iskra/contracts";
...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/iskra/environment")) {
  ...
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(calls.map((call) => call.url)).toEqual([
        "https://remote.example.test/.well-known/iskra/environment",
      ...
      expect(tokenParams.get("client_label")).toBe("Iskra Test");
    }),
  );
