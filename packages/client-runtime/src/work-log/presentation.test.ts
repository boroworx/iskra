...
describe("resolveWorkEntryToolPresentation", () => {
  it.each([
    "mcp__iskra__preview_click",
    "mcp__iskra_code__preview_click",
    "mcp__iskra__preview_click",
    "Iskra.preview_click",
    "iskra · preview_click completed",
    "iskra_code/preview_click",
    "preview_click",
  ...
  it("labels device tools with the device icon", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__iskra__device_open",
        toolLifecycleStatus: "completed",
      }),
    ).toEqual({ displayName: "Opened a device in the Device panel", icon: "device" });
    expect(resolveWorkEntryToolPresentation({ label: "iskra · device_screenshot" })).toEqual({
  ...
  it("uses structured MCP identity when the provider supplies a custom title", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolData: { server: "iskra", tool: "preview_snapshot", result: { title: "Example" } },
      }),
  ...
  it("uses the summary's state only when the provider omitted a lifecycle status", () => {
    const entry = { label: "Iskra.preview_click" };
  ...
  it.each([
    ...
    ["iskra_thread_read", "Reading an Iskra thread", "Read an Iskra thread"],
    ["iskra_thread_send", "Sending to an Iskra thread", "Sent to an Iskra thread"],
    [
      "iskra_worktree_handoff",
  ...
  it("keeps Iskra branding for non-browser tools and falls back to the original tool label", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__iskra_code__task_status",
        toolTitle: "Check the child task",
      }),
    ).toEqual({ displayName: "Getting delegated task status", icon: "iskra" });
  });
...
describe("browser group summaries", () => {
  const browserEntry: WorkLogPresentationEntry = {
    label: "MCP tool call",
    toolData: { server: "iskra", tool: "preview_click" },
  ...
  it("preserves first-seen action ordering alongside non-browser tools", () => {
    expect(
      summarizeToolGroup([
        ...
        {
          ...browserEntry,
          toolData: { server: "iskra", tool: "task_status" },
        },
      ]),
      ...
      summarizeToolGroup([
        {
          ...
          toolData: { toolName: "mcp__iskra_code__preview_evaluate" },
        },
      ]),
...
describe("resolveViewedImageAsset", () => {
  ...
  it("serves iskra attachment paths in place like any other host path", () => {
    const path = "/Users/demo/.iskra/dev/attachments/11111111-1111-4111-8111-111111111111.png";
...
describe("pull request tool presentation", () => {
  it.each([
    "mcp__iskra__link_pull_request",
    "mcp__iskra_code__link_pull_request",
    "Iskra · link_pull_request",
    "iskra/link_pull_request",
    "link_pull_request",
  ...
  it("recognizes unlink targets supplied as repository and number", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolLifecycleStatus: "completed",
        toolData: {
          toolName: "mcp__iskra__unlink_pull_request",
  ...
  it("summarizes native PR work separately from ordinary tools and integration metadata", () => {
    const link: WorkLogPresentationEntry = {
      label: "Iskra · link_pull_request",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolLifecycleStatus: "completed",
      toolSource: { key: "iskra", name: "Iskra", kind: "integration" },
    };
    const list: WorkLogPresentationEntry = {
      ...link,
      label: "Iskra · list_thread_pull_requests",
    };
    expect(summarizeToolGroup([link, link, list])).toBe(
      "Linked 2 pull requests and checked linked pull requests",
    );
    expect(summarizeToolGroup([{ ...link, label: "Iskra · unlink_pull_request" }])).toBe(
...
describe("device group summaries", () => {
  ...
  it("recognizes Claude tool names and preserves screenshot previews", () => {
    const entry = {
      ...
      toolData: { toolName: "mcp__iskra_code__device_screenshot" },
