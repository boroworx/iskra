...
describe("undoCommandOf", () => {
  it("reverses each command a toast offers to undo", () => {
    ...
    expect(undoCommandOf({ type: "card.abandon", cardId })).toEqual({
      type: "card.reopen",
      cardId,
    });
    expect(undoCommandOf({ type: "card.unapprove", cardId })).toEqual({
      type: "card.approve",
      cardId,
    });
    expect(undoCommandOf({ type: "card.snooze", cardId })).toEqual({
      type: "card.unsnooze",
      cardId,
    });
    expect(
