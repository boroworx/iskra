import { EnvironmentId, ProjectId } from "@iskra/contracts";
...
describe("buildProjectGroups", () => {
  it("preserves every physical clone as a selectable member in repository modes", () => {
    const projects = [
      makeProject("iskra", "/work/iskra"),
      makeProject("iskra-2", "/work/iskra-2"),
      makeProject("iskra-3", "/work/iskra-3"),
    ];
  ...
  it("uses a shared custom title as the repository group's label", () => {
    const projects = [
      makeProject("first", "/work/iskra", { title: "Custom project" }),
      makeProject("second", "/work/iskra-2", { title: "Custom project" }),
    ];
  ...
  it("keeps the repository label when shared titles match its repository name", () => {
    const projects = [
      makeProject("first", "/work/iskra", { title: "iskra" }),
      makeProject("second", "/work/iskra-2", { title: "iskra" }),
    ];
  ...
  it("keeps physical clones in separate groups when requested", () => {
    ...
    expect(groups.map((group) => group.label)).toEqual(["iskra", "iskra-2", "iskra-3"]);
  });

  it("applies a physical-project override without dropping its siblings", () => {
    const first = makeProject("iskra", "/work/iskra");
    const second = makeProject("iskra-2", "/work/iskra-2");
    const third = makeProject("iskra-3", "/work/iskra-3");
  ...
  it("dedupes stale registrations at one physical path using the freshest project", () => {
    const stale = makeProject("stale", "/work/iskra", {
      repositoryIdentity: null,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const fresh = makeProject("fresh", "/work/iskra/", {
  ...
  it("uses repository identity from a duplicate registration when the winner lacks it", () => {
    const identified = makeProject("identified", "/work/iskra", {
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshUnidentified = makeProject("fresh", "/work/iskra/", {
      repositoryIdentity: null,
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const sibling = makeProject("sibling", "/work/iskra-2");
  ...
  it("uses the freshest winner's repository identity when stale duplicates disagree", () => {
    const staleIdentity = {
      ...repositoryIdentity,
      canonicalKey: "github.com/boroworx/old-repository",
      name: "old-repository",
  ...
  it("uses the freshest identity-bearing duplicate when the winner lacks identity", () => {
    ...
    const staleIdentified = makeProject("stale-identified", "/work/iskra", {
      repositoryIdentity: staleIdentity,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshIdentified = makeProject("fresh-identified", "/work/iskra/", {
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const winner = makeProject("winner", "/work/iskra", {
      repositoryIdentity: null,
