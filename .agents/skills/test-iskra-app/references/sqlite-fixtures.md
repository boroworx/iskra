...
Load this reference only when inspecting or seeding local Iskra state directly.
...
```bash
node apps/server/scripts/iskra-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/iskra-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/iskra-seed.sql
```
